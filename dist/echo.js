...
const DATA_CURRENCY_NOTE = "ECHO Version 'ALL DATA v2017-06-16' is the schema-version label, not a data cutoff; data is refreshed weekly and current through 2026 (live-verified).";
const FACILITY_NAME_NOTE = "facility-name filter (p_fn) is not validated by ECHO — a typo silently returns 0 results, not an error; verify spelling if the result is empty.";
const NAICS_BEST_EFFORT_NOTE = "ECHO did not narrow by NAICS at this (or any) scale — the p_naics filter is dropped upstream (live-verified 2026-07-12) — so the returned facilities are NOT guaranteed to match the NAICS code; naics is listed in _meta.filtersDropped. Use `sic` (which DOES narrow), facilityName, majorOnly, or federalOnly to scope by industry/type, and verify NAICS client-side.";
const NUMERIC_STRING_NOTE = "Count/amount fields arrive as strings; QueryRows is coerced for the exact total, but row-level currency fields (e.g. TotalPenalties '$1,056,616') keep their $/comma formatting — parse client-side. A missing value is null, never 0.";
const TWO_STEP_NOTE = "Results are the ECHO all-program facility compliance search (CAA/CWA/RCRA/SDWA); totalAvailable is the EXACT QueryRows total; rows are fetched via a hidden two-step QueryID pagination (the QueryID is ephemeral/globally-recycled and never exposed).";
 ...
 */
export async function echoGet(service, params) {
...
function unwrapResults(service, body) {
    const r = body?.Results;
    if (r === null || typeof r !== "object" || Array.isArray(r)) {
        throw driftError("echo:" + service, `echo:${service} returned an unexpected shape (missing Results object).`);
    }
    return r;
}
/**
 ...
 * never sees it.
 */
function guardResultsError(service, results) {
    const err = results.Error;
    if (err === undefined || err === null)
        return;
    const label = "echo:" + service;
    const msg = str(err.ErrorMessage) ??
    ...
    throw driftError(label, `echo:${service} returned an unrecognized Results.Error — treating as schema drift. Upstream: ${msg}`);
}
...
export async function searchFacilities(args) {
    ...
    if (args.naics !== undefined)
        p1.set("p_naics", args.naics);
    if (args.sic !== undefined)
        p1.set("p_sic", args.sic);
    if (args.facilityName !== undefined)
        p1.set("p_fn", args.facilityName);
    if (args.majorOnly === true)
        p1.set("p_maj", "Y");
    if (args.federalOnly === true)
        p1.set("p_ff", "Y");
    ...
    const total = num(results1.QueryRows);
    if (total === null) {
        throw driftError("echo:" + SVC_FACILITIES, `echo:${SVC_FACILITIES} returned no numeric QueryRows and no Results.Error — treating as schema drift.`);
    }
    // Program-count summary (all pass-through; TotalPenalties is a verbatim currency
    // string that num() cannot parse — kept as-is).
    const summary = {
        queryRows: total,
        totalPenalties: str(results1.TotalPenalties),
        programCounts: {
            caa: num(results1.CAARows),
            cwa: num(results1.CWARows),
            rcra: num(results1.RCRRows),
            tri: num(results1.TRIRows),
            inspections: num(results1.INSPRows),
        },
    };
    ...
    if (args.sic !== undefined)
        filtersApplied.push("sic");
    if (args.facilityName !== undefined)
        filtersApplied.push("facilityName");
    if (args.majorOnly === true)
        filtersApplied.push("majorOnly");
    if (args.federalOnly === true)
        filtersApplied.push("federalOnly");
    ...
    if (total === 0) {
        return withMeta({ state: args.state, facilities: [], summary }, {
            source: SOURCE,
            keylessMode: true,
            returned: 0,
            totalAvailable: 0,
    ...
    const qid = str(results1.QueryID);
    if (qid === null || !QID_RE.test(qid)) {
        throw driftError("echo:" + SVC_FACILITIES, `echo:${SVC_FACILITIES} returned a missing/non-numeric QueryID — cannot fetch the result page (schema drift).`);
    }
    ...
    const facilities = results2.Facilities;
    if (!Array.isArray(facilities)) {
        throw driftError("echo:" + SVC_QID, `echo:${SVC_QID} returned a non-array Facilities with no Results.Error — treating as schema drift (never a fake empty).`);
    }
    const returned = facilities.length;
    // hasMore is EXACT (QueryRows is an exact total — no page-fullness hedge).
    const hasMore = offset + returned < total;
    const nextOffset = hasMore ? offset + returned : null;
    return withMeta({ state: args.state, facilities: facilities, summary }, {
        source: SOURCE,
        keylessMode: true,
        returned,
...
export async function facilityReport(args) {
    ...
    return withMeta({
        registryId: str(results.RegistryID) ?? args.registryId,
        report: results,
    }, {
        source: SOURCE,
        keylessMode: true,
