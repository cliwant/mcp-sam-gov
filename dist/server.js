...
const SamAttachmentUrlInput = z.object({
    resourceId: z
        .string()
        .describe("Resource id from getOpportunity → resourceLinks (the URL-tail hex)"),
});
...
const TOOLS = [
    {
        ...
        description: "Search SAM.gov federal contracting opportunities (keyless). Returns up to 50 active notices with title, agency, NAICS, and noticeId. Use to discover pursuits matching NAICS / agency / set-aside / state filters.",
        inputSchema: SamSearchInput,
    },
    {
        name: "sam_get_opportunity",
        description: "Fetch full detail for a single SAM.gov notice by 32-char hex noticeId. Returns title, agency, solicitation #, POCs, response deadline, attachments, and inline description body. Use BEFORE drafting bid/no-bid or compliance work.",
        inputSchema: SamGetOpportunityInput,
    },
    {
        name: "sam_fetch_description",
        description: "Return the full description / RFP body text for a notice as plain text. Useful when sam_get_opportunity returned a description URL instead of inline body, or when you need an LLM-friendly text dump.",
    ...
    },
    {
        ...
        description: "Aggregate share-of-wallet on USAspending. Given an agency × NAICS × fiscal year, returns top recipients by total $ + count. Use for competitive landscape questions ('who wins at VA in 541512?').",
        inputSchema: UsasSearchAwardsInput,
    },
    {
        name: "usas_search_individual_awards",
        description: "Line-item federal contracts on USAspending. Returns specific awards (recipient + $ + sub-agency + state + description). Use AFTER usas_search_awards when the user wants 'show me the actual contracts'.",
        inputSchema: UsasSearchIndividualInput,
    },
    {
        name: "usas_search_subagency_spending",
        description: "Break down a parent agency's spending by sub-agency / office. Surfaces which office controls the budget (e.g. VA OI&T vs VHA, DoD vs Army vs DISA).",
...
async function main() {
    ...
    // eslint-disable-next-line no-console
    console.error(`[govicon-mcp-sam-gov] v${SERVER_VERSION} listening on stdio (${TOOLS.length} tools).`);
}
async function runTool(name, args, sam) {
    switch (name) {
        ...
        case "usas_search_individual_awards":
            return await searchIndividualAwards(UsasSearchIndividualInput.parse(args));
