...
const TableauViewCsvInput = z.object({
    view: z
        .enum(tableau.TABLEAU_VIEWS.map((v) => v.key))
...
const ArcgisFeatureQueryInput = z.object({
    service: z
        ...
        .describe("Curated ArcGIS service key (SSRF allowlist; 27 services). DC OCP PASS (solicitations/contracts/purchase_orders/payments). US local govs: Asheville NC, Bellevue WA, Miami-Dade FL×2, Suffolk County NY, Mat-Su AK, Las Vegas NV×2, Baltimore MD, Naperville IL, Worcester MA, Topeka KS; TX/AK/IA/OK DOT bid/award registers. ND DOT flex-funding to local agencies (nddot_flex×4 — NOT vendor contracts)."),
    where: z
        .string()
        .min(1)
        .max(500)
...
const CensusBusinessPatternsInput = z.object({
    ...
    year: z
        ...
        .regex(/^\d{4}$/)
        .optional()
        .describe("The CBP data year (default '2023', the latest published vintage — CBP is released with a ~2-year lag). Validated ^\\d{4}$ (it rides in the request path)."),
    limit: z
        .number()
        .int()
        .min(0)
...
export const TOOLS = [
    ...
    defineTool({
        name: "bls_oews_wages",
        description: "BLS OEWS occupational wage/employment benchmarking by area × occupation × datatype (keyless; api.bls.gov). Builds validated 25-char series IDs from structured inputs and batches them into one POST — NO year input (OEWS serves only the latest annual release). Inputs: `occupation` (curated enum, e.g. 'software_developer') or `soc` (raw 6-digit SOC, ^[0-9]{6}$ NO hyphen — at least ONE required); `area` (default 'national', 2-letter USPS state, or 5-digit CBSA metro code); `datatype` (default 'annual_mean'; annual_mean/annual_median/hourly_mean/hourly_median/employment). Returns { rows:[{ area:{type,code,label}, occupation:{soc,key,label}, measure:{key,code,units}, value:number|null, valueUnavailable:bool, referenceYear, referencePeriod, footnotes, seriesId }] } + honest _meta. ★H1: OEWS is an ANNUAL point-in-time snapshot (reference May <year>); BLS API serves ONLY the most recent release, may lag ~1 year. NOT monthly/current-quarter. Historical OEWS requires the downloadable tables (bls.gov/oes/tables.htm). ★H2: a built-ID with no published value (occupation not surveyed or suppressed in that area) → value:null (NOT a tool error). ★H3: measure.units is set from datatype — annual_mean/annual_median=dollars/year; hourly_mean/hourly_median=dollars/hour; employment=count. NEVER mislabeled. ★H4: '#' top-code → value:null + valueUnavailable + footnote (never fabricated). BLS '-' marker → value:null (never 0) + valueUnavailable:true + footnote. Active tier (v1 keyless ~25/day or v2 BLS_API_KEY ~500/day) and series-cap limits disclosed.",
...
export function humanizeToolTitle(name) {
