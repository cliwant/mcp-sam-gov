...
const FDIC_FAILURES_FILTER_FIELDS = new Set(["PSTALP", "FAILYR", "CERT"]);
...
const FAIL_SORT_FIELDS = new Set([
    "FAILDATE",
    "COST",
    "QBFASSET",
    "QBFDEP",
 ...
 */
export function normFailDate(raw) {
    ...
    if (typeof raw !== "string")
        return { value: str(raw), normalized: false };
...
const FAIL_COST_NOTE = "COST is FDIC's estimated loss to the Deposit Insurance Fund; COST/QBFDEP/QBFASSET are $thousands, normalized here to whole USD (×1,000). A genuine 0 = a fully-assisted resolution with NO DIF loss (a real 0, not absence); a NEGATIVE value = a net DIF recovery/gain (NOT a loss); an absent value is null (never 0).";
const FAIL_SCOPE_NOTE = "Historical FDIC-insured institution failures / assistance transactions; a bank ABSENT here has no recorded FDIC failure (it may be active, acquired non-failed, or never FDIC-insured) — cross-check with fdic_search_institutions.";
const FAIL_CERT_NOTE = "To find a specific institution's failure, resolve its CERT via fdic_search_institutions, then filter here by `cert`; name/city are shown but not searchable on this endpoint (FDIC's /failures `search` param is ignored and would return the whole dataset).";
/**
...
export async function bankFailures(args) {
    ...
    const sort = sortParams(args.sortBy ?? "FAILDATE", args.sortOrder ?? "DESC", FAIL_SORT_FIELDS, "failures");
    ...
    if (args.state !== undefined)
        filtersApplied.push("state");
    if (args.failYear !== undefined)
        filtersApplied.push("failYear");
    if (args.cert !== undefined)
        filtersApplied.push("cert");
    if (sort.sort_by)
        filtersApplied.push("sort");
    ...
    if (anyRawFailDate) {
        notes.push("One or more FAILDATE values did not match FDIC's M/D/YYYY format (or failed the calendar round-trip) and were surfaced RAW (not normalized to ISO) — never nulled or fabricated.");
    }
    const fu = fieldsUnavailable(env.records, FAIL_PROJECTION);
    if (fu.length > 0) {
        notes.push(`Requested field(s) ${fu.join(", ")} were not returned by FDIC for any record — possible schema drift / rename.`);
    }
    return withMeta({ failures: records }, {
...
const HIST_FIELDS = "CERT,INSTNAME,PSTALP,CHANGECODE,CHANGECODE_DESC,EFFDATE,PROCDATE,EFFYEAR,TRANSNUM,ACQ_CERT,ACQ_INSTNAME,OUT_CERT,OUT_INSTNAME,SUR_CERT,SUR_INSTNAME,ID";
...
const FDIC_HISTORY_FILTER_FIELDS = new Set([
    "CERT",
    "CHANGECODE",
    "EFFYEAR",
    "PSTALP",
...
// → 400, so the pre-fetch guard is load-bearing).
const HIST_SORT_FIELDS = new Set([
    "EFFDATE",
    "PROCDATE",
    "CHANGECODE",
    "TRANSNUM",
 ...
 */
export function normHistDate(raw) {
    // A non-string (absent/null/numeric) → honest str coercion (null for absent);
    // never String()-fabricate "null"/"undefined"/"[object Object]".
    if (typeof raw !== "string")
        return { value: str(raw), normalized: false };
...
const HIST_DATE_NOTE = "effectiveDate/processDate are normalized from FDIC's YYYY-MM-DDT00:00:00 to ISO YYYY-MM-DD.";
const HIST_CHANGEDESC_NOTE = "changeDescription is FDIC's own CHANGECODE_DESC, co-served with the numeric code in each record; the numeric changeCode is authoritative.";
const HIST_COUNTERPARTY_NOTE = "acquirer/outgoing/surviving identify the merger counterparties; each Cert links back to fdic_search_institutions / fdic_institution_financials / fdic_bank_failures; null = no counterparty for this event type.";
const HIST_SCOPE_NOTE = "Structural-change events for FDIC-insured institutions; a bank ABSENT here (for a given CERT) has no recorded structural change — cross-check with fdic_search_institutions. Name/city are surfaced (instName + counterparty names) but NOT searchable on this endpoint (FDIC's /history `search` param returns 0 for INSTNAME); to find an institution's history, resolve its CERT via fdic_search_institutions, then filter here by `cert`.";
/**
...
export async function institutionHistory(args) {
    ...
    const sort = sortParams(args.sortBy ?? "EFFDATE", args.sortOrder ?? "DESC", HIST_SORT_FIELDS, "history");
    ...
    if (args.cert !== undefined)
        filtersApplied.push("cert");
    if (args.changeCode !== undefined)
        filtersApplied.push("changeCode");
    if (args.effYear !== undefined)
        filtersApplied.push("effYear");
    if (args.state !== undefined)
        filtersApplied.push("state");
    if (sort.sort_by)
        filtersApplied.push("sort");
    ...
    const anyRawDate = env.records.some((rec) => ["EFFDATE", "PROCDATE"].some((f) => {
        const nd = normHistDate(rec[f]);
        return !nd.normalized && nd.value !== null && nd.value !== "";
    }));
    if (anyRawDate) {
        notes.push("One or more effectiveDate/processDate values did not match FDIC's YYYY-MM-DDT00:00:00 format (or failed the calendar round-trip) and were surfaced RAW (not normalized to ISO) — never nulled or fabricated.");
    }
    ...
    const anySentinel = env.records.some((rec) => ["EFFDATE", "PROCDATE"].some((f) => typeof rec[f] === "string" && rec[f].startsWith("9999")));
    if (anySentinel) {
        notes.push("One or more effectiveDate/processDate values carry FDIC's 9999-* 'not-applicable / open' sentinel — surfaced verbatim, not treated as a real event date.");
    }
    const fu = fieldsUnavailable(env.records, HIST_PROJECTION);
    if (fu.length > 0) {
        notes.push(`Requested field(s) ${fu.join(", ")} were not returned by FDIC for any record — possible schema drift / rename.`);
    }
    return withMeta({ history: records }, {
...
// the wire projection.
const RATIO_CODES = RATIO_CATALOG.map((e) => e.code);
...
const RATIO_PROJECTION = ["CERT", "REPDTE", "CBLRIND", ...RATIO_CODES, "ID"];
...
const RATIO_FILTER_FIELDS = new Set(["CERT", "REPDTE"]);
// sortBy allowlist (mirrors the server's Zod enum; a Set.has recheck in sortParams →
// an unknown sort field is invalid_input BEFORE fetch).
const RATIO_SORT_FIELDS = new Set(["REPDTE", "ROA", "ROE", "RBCRWAJ", "EEFFR"]);
 ...
 */
export function buildRatioFilters(inp) {
    const terms = [];
 ...
 */
export function isCblrFramework(rec) {
...
export async function riskRatios(args) {
    ...
    const sort = sortParams(args.sortBy ?? "REPDTE", args.sortOrder ?? "DESC", RATIO_SORT_FIELDS, "financials");
    const params = new URLSearchParams();
    ...
    if (args.reportDate !== undefined)
        filtersApplied.push("reportDate");
    if (sort.sort_by)
        filtersApplied.push("sort");
    const notes = [
        freshnessNote(env.indexName, env.indexCreated),
    ...
    if (totalAvailable === 0)
        notes.push(RATIO_EMPTY_NOTE);
    const fu = fieldsUnavailable(env.records, RATIO_PROJECTION);
    if (fu.length > 0) {
        notes.push(`Requested field(s) ${fu.join(", ")} were not returned by FDIC for any record in this result set — the affected values are surfaced as null (never fabricated); this can be a ratio FDIC does not publish for these institution(s)/period(s), or a schema change.`);
    }
    return withMeta({ cert: args.cert, ratios: records }, {
        source: "api.fdic.gov/banks/financials (BankFind, keyless)",
        keylessMode: true,
        returned,
        totalAvailable,
...
const SOD_FILTER_FIELDS = new Set(["CERT", "STALPBR", "YEAR"]);
// sortBy allowlist (mirrors the server's Zod enum; a Set.has recheck in sortParams →
// an unknown sort field is invalid_input BEFORE fetch; live `sort_by=NOTAFIELD` → 400).
const SOD_SORT_FIELDS = new Set(["YEAR", "DEPSUMBR"]);
 ...
 * Exported for the fault fixtures.
 */
export function buildSodFilters(inp) {
    const terms = [];
    if (inp.cert !== undefined)
        terms.push(filterTerm("CERT", String(inp.cert), SOD_FILTER_FIELDS));
    if (inp.state !== undefined)
        terms.push(filterTerm("STALPBR", inp.state, SOD_FILTER_FIELDS));
    if (inp.year !== undefined)
        terms.push(filterTerm("YEAR", String(inp.year), SOD_FILTER_FIELDS));
    return terms.join(" AND ");
}
function mapBranchDeposit(rec) {
    return {
        cert: num(rec.CERT),
...
const SOD_UNITS_NOTE = "depositsUSD is FDIC's DEPSUMBR (branch-office deposits), published in $thousands and normalized here to whole USD (×1,000). A real 0 stays 0; an absent value is null (never 0).";
const SOD_SNAPSHOT_NOTE = "Summary of Deposits is an ANNUAL June-30 branch-office snapshot (a DISTINCT index, far less fresh than the quarterly /financials data). Branch name/city/address are shown per row but the endpoint filters only by cert/state/year — resolve a bank's CERT via fdic_search_institutions.";
const SOD_EMPTY_NOTE = "No Summary-of-Deposits branch records match — the bank may report no branches for this year, or the CERT/state/year filter may not match; SOD is an annual June-30 snapshot. Confirm the CERT via fdic_search_institutions.";
/**
...
export async function branchDeposits(args) {
    ...
    if (totalAvailable === 0)
        notes.push(SOD_EMPTY_NOTE);
    const fu = fieldsUnavailable(env.records, SOD_PROJECTION);
    if (fu.length > 0) {
        notes.push(`Requested field(s) ${fu.join(", ")} were not returned by FDIC for any record — possible schema drift / rename.`);
    }
    return withMeta({ branches: records }, {
