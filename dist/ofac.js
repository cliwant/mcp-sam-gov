...
export function parsePrimaryList(body, list, label) {
    ...
    for (const rec of records) {
        ...
        if (rec.length !== PRIMARY_COLS) {
            throw driftError(label, `OFAC ${label} content row ${rowNo} has ${rec.length} column(s), expected ${PRIMARY_COLS} — the download was truncated or the file schema drifted. Refusing to screen a drifted list (a near-empty parse must never read as a clear).`);
        }
...
export function parseAltList(body, list, label) {
    ...
    for (const rec of records) {
        ...
        if (rec.length !== ALT_COLS) {
            throw driftError(label, `OFAC ${label} content row ${rowNo} has ${rec.length} column(s), expected ${ALT_COLS} — the download was truncated or the file schema drifted. Refusing to screen a drifted alias list (missing an AKA would be a false clear).`);
        }
