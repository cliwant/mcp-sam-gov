...
const BENCHMARK_VINTAGE_NOTE = "vintage must be COMPATIBLE with the chosen benchmark (the valid vintage set is a (benchmark, vintage) matrix); an incompatible pair fails-closed with an HTTP 400 (invalid_input), never a silent mis-resolution.";
const GEOID_STRING_NOTE = "GEOIDs are STRINGS with real leading zeros (State '01', County '01101', Tract '01101000200', CD '0102') — they are never numeric (a leading zero must survive); a missing value is null, never an empty string.";
 ...
 */
export async function censusGet(path, params) {
 ...
 */
export function mapGeographies(geographies, label, vintageName) {
    ...
    for (const spec of LAYER_SPECS) {
        ...
        if (matched.length === 0) {
            ...
            if (spec.sentinel && nonEmpty) {
                throw driftError(label, `census: expected sentinel layer "${spec.label}" matched ZERO keys in a non-empty geographies object (keys: ${JSON.stringify(allKeys)}) — treating as schema drift (the layer-key suffix may have changed).`);
            }
            geo[spec.field] = null;
            continue;
        }
        ...
        const layers = ordered.map((k) => toGeoLayer(k, geographies[k][0]));
        ...
        if (layers.length > 1) {
            ...
            notes.push(`vintage ${vintageName ?? "(unknown)"} returned ${layers.length} "${spec.label}" layers: ${ordered
...
function buildAddressMatch(m, label, vintageName) {
    const coordsRaw = (m.coordinates ?? {});
    const tiger = (m.tigerLine ?? {});
    const ac = (m.addressComponents ?? {});
    const g = m.geographies;
    const geoObj = g !== null && typeof g === "object" && !Array.isArray(g)
        ? g
        : {};
    ...
    const street = [str(ac.fromAddress), str(ac.streetName), str(ac.suffixType)]
        .filter((p) => p !== null)
        .join(" ") || null;
    return {
        match: {
...
function resolvedVintage(input) {
...
function guardBenchmark(benchmark) {
    if (!CENSUS_BENCHMARKS.includes(benchmark)) {
        throw new ToolErrorCarrier({
            kind: "invalid_input",
...
function guardVintage(vintage) {
    if (!CENSUS_VINTAGES.includes(vintage)) {
        throw new ToolErrorCarrier({
            kind: "invalid_input",
...
export async function geocodeAddress(args) {
    ...
    if (result === null || typeof result !== "object" || Array.isArray(result)) {
        throw driftError("census:" + PATH_ONELINE, "census: onelineaddress returned an unexpected shape (missing result object).");
    }
    const r = result;
    const addressMatches = r.addressMatches;
    if (!Array.isArray(addressMatches)) {
        throw driftError("census:" + PATH_ONELINE, "census: onelineaddress returned a non-array addressMatches with no error — treating as schema drift (never a fake empty).");
    }
    const res = resolvedVintage(r.input);
    const notes = [movingVintageNote(res), HUBZONE_OZ_CAVEAT, BENCHMARK_VINTAGE_NOTE, GEOID_STRING_NOTE];
    ...
    if (addressMatches.length === 0) {
        notes.push("No address match — the address did not geocode. Verify spelling and add city, state, and ZIP (an under-specified address returns 0, a genuine empty, not an error).");
        return withMeta({
            matches: [],
            matchCount: 0,
            benchmark,
    ...
    for (const m of addressMatches) {
        const built = buildAddressMatch((m ?? {}), "census:" + PATH_ONELINE, res.vintage);
    ...
    if (matches.length > 1) {
        notes.push(`${matches.length} addresses matched — the geographies are listed PER match; matchedAddress shows what each resolved to. Refine the address (add ZIP / unit / state) if the intended location is ambiguous.`);
    }
    return withMeta({
        matches,
...
export async function geographiesByCoordinates(args) {
    ...
    const result = body?.result;
    if (result === null || typeof result !== "object" || Array.isArray(result)) {
        throw driftError("census:" + PATH_COORDINATES, "census: coordinates returned an unexpected shape (missing result object).");
    }
    const r = result;
    ...
    if (geographiesRaw === null ||
        typeof geographiesRaw !== "object" ||
        Array.isArray(geographiesRaw)) {
        throw driftError("census:" + PATH_COORDINATES, "census: coordinates returned a non-object geographies with no error — treating as schema drift (never a fake empty).");
    }
    const res = resolvedVintage(r.input);
    const loc = r.input?.location;
    const coordinates = {
        x: num(loc?.x) ?? xVal,
        y: num(loc?.y) ?? yVal,
    };
    const notes = [movingVintageNote(res), HUBZONE_OZ_CAVEAT, BENCHMARK_VINTAGE_NOTE, GEOID_STRING_NOTE];
    const geoObj = geographiesRaw;
    ...
    if (Object.keys(geoObj).length === 0) {
        const empty = {};
        for (const spec of LAYER_SPECS)
            empty[spec.field] = null;
        notes.push("The point is not within a US Census geography (offshore / outside the US) — an honest empty (geographies:{}), not an error.");
        return withMeta({
            found: false,
            coordinates,
            geographies: empty,
    ...
    const { geo, notes: multiKeyNotes } = mapGeographies(geoObj, "census:" + PATH_COORDINATES, res.vintage);
    for (const n of multiKeyNotes)
        if (!notes.includes(n))
            notes.push(n);
    return withMeta({
        found: true,
