...
const DOCKET_SUIT_NATURE_NOTE = "natureOfSuit was applied as a fielded suitNature:\"…\" query — this IS a real filterable field on dockets (unlike opinions where it is folded into free-text q).";
/** Map ONE RECAP docket result row → curated CourtlistenerDocket shape. */
function mapDocket(raw) {
    const o = (raw ?? {});
    return {
...
export async function searchDockets(args) {
    ...
    try {
        ...
        if (e instanceof SyntaxError) {
            throw driftError(label, "CourtListener /api/rest/v4/search (type=r) returned a non-JSON body at HTTP 200 — schema drift.");
        }
        if (e instanceof ToolErrorCarrier && e.toolError.upstreamStatus === 400) {
            const apiMsg = await readCourtlistenerErrorMessage(url, headers);
            throw new ToolErrorCarrier({
    ...
    const b = (body ?? {});
    if (!Array.isArray(b.results)) {
        throw driftError(label, "CourtListener /api/rest/v4/search (type=r) shape drift — `results` must be an array.");
    }
    const rawCount = b.count;
    const countIsNumber = typeof rawCount === "number" && Number.isFinite(rawCount);
    if (!countIsNumber && rawCount !== null && rawCount !== undefined) {
        throw driftError(label, "CourtListener /api/rest/v4/search (type=r) shape drift — `count` must be a number or null.");
    }
    const dockets = b.results.map(mapDocket);
    const returned = dockets.length;
    const totalAvailable = countIsNumber ? rawCount : null;
    const nextCursor = extractNextCursor(b.next, label);
    const hasMore = nextCursor !== null;
    const notes = [DOCKET_PROVENANCE_NOTE, COUNT_TOTAL_NOTE];
    if (!countIsNumber)
        notes.push(DEEP_PAGE_NO_COUNT_NOTE);
    if (args.natureOfSuit !== undefined && args.natureOfSuit !== "") {
