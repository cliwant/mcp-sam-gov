...
export const OPENFDA_CATEGORIES = ["drug", "device", "food"];
const OPENFDA_CATEGORY_SET = new Set(OPENFDA_CATEGORIES);
// The FDA recall classification enum (surfaced verbatim; a structured filter value).
export const OPENFDA_CLASSIFICATIONS = ["Class I", "Class II", "Class III"];
const OPENFDA_CLASSIFICATION_SET = new Set(OPENFDA_CLASSIFICATIONS);
...
const NOT_DETERMINATION_NOTE = "openFDA recall/enforcement records are FDA-published recall reports; treat classification/status/dates as of the source's last publication. This is reference data, not a live regulatory determination.";
 ...
 */
export function buildSearch(f) {
    const clauses = [];
    if (f.firm !== undefined)
        clauses.push(`recalling_firm:${luceneQuote(f.firm)}`);
    ...
    if (f.status !== undefined)
        clauses.push(`status:${luceneQuote(f.status)}`);
...
export async function fetchOpenfda(url, label) {
    ...
    try {
        ...
        if (isRedirectError(e)) {
            throw driftError(label, `openFDA returned an off-host redirect (redirect:"error") while fetching ${label} — refusing to follow it (SSRF safety).`);
        }
 ...
 */
export async function enforcement(args) {
    ...
    if (args.firm !== undefined)
        filtersApplied.push("firm");
    if (args.product !== undefined)
        filtersApplied.push("product");
    if (args.reason !== undefined)
        filtersApplied.push("reason");
    if (args.classification !== undefined)
        filtersApplied.push("classification");
    ...
    try {
        ...
        if (e instanceof SyntaxError) {
            throw driftError(label, `openFDA ${label} returned a non-JSON body at HTTP 200 — schema drift (never read as an empty result).`);
        }
        throw e;
    }
    ...
    if (metaResults === undefined ||
        ...
        throw driftError(label, `openFDA ${label} shape drift — meta.results (the skip/limit/total carrier) is missing.`);
    }
    if (!Array.isArray(b.results)) {
        throw driftError(label, `openFDA ${label} shape drift — results must be an array.`);
    }
...
export function openfdaPageMeta(skip, returned, totalAvailable) {
...
export function openfdaEmptyTotal(skip) {
    return skip === 0 ? 0 : null;
}
/** Disclosure for a skip>0 404 (ambiguous over-skip vs no-match; total unknown). */
export const OPENFDA_OVERSKIP_NOTE = "Returned 0 at this offset: the skip is at/past the end of the result set (or a genuine no-match). openFDA returns an IDENTICAL HTTP 404 for both, so the true total is UNKNOWN at a non-zero skip (totalAvailable is null, not 0) — re-query at skip:0 for the exact total. This is NOT a confirmed empty result set.";
