...
const BROWSER_UA = "Mozilla/5.0 (compatible; @cliwant/mcp-sam-gov; +https://github.com/cliwant/mcp-sam-gov)";
 ...
 */
function assertSamAttachmentUrl(url) {
    let parsed;
    try {
        parsed = new URL(url);
    }
 ...
 */
function detectFormat(bytes, contentType, ext) {
...
function sniffLeadingText(bytes, n) {
...
function extractDocxText(bytes) {
    ...
    const u32 = (o) => dv.getUint32(o, true); // little-endian
    const u16 = (o) => dv.getUint16(o, true);
    const utf8 = (start, end) => new TextDecoder("utf-8", { fatal: false }).decode(bytes.subarray(start, end));
    ...
    if (n < 22)
        throw new Error("not a valid DOCX (file too small to be a ZIP)");
    ...
    if (!target) {
        throw new Error("not a valid Word document (word/document.xml not found in the ZIP)");
    }
 ...
 */
export async function fetchAttachmentText(args) {
    const parsed = assertSamAttachmentUrl(args.url);
    const url = parsed.toString();
    const maxChars = Math.min(Math.max(1, args.maxChars ?? DEFAULT_MAX_CHARS), MAX_MAX_CHARS);
    ...
    try {
        ...
        const err = {
            ...
            message: `Network error fetching SAM attachment ${url}: ${e.message}. The service is unavailable, NOT an empty attachment — retry.`,
            retryable: true,
    ...
    if (format === "pdf") {
        try {
            ...
            text = null;
            pages = null;
            notes.push(`PDF text extraction failed (extractionError): ${e.message}. The file may be encrypted, corrupt, or image-only (scanned). It was NOT read; download it at ${url}.`);
        }
    }
    else if (format === "docx") {
        ...
        try {
            ...
            text = null;
            pages = null;
            notes.push(`DOCX text extraction failed: ${e.message}. The file may be corrupt or not a valid Word document — download it at ${url}.`);
        }
    }
    else if (format === "html") {
        ...
        text = null;
        pages = null;
        notes.push(`${format} content is not extractable keyless in this tool — download it at ${url}.`);
    }
    ...
    if (text != null) {
        const replacementShare = (text.match(/\uFFFD/g)?.length ?? 0) / Math.max(1, text.length);
        if (text.trim().length === 0) {
            notes.push(format === "pdf"
            ...
            notes.push(`The ${format} attachment could not be decoded as readable UTF-8 text (it likely uses a different character set, or is binary mislabeled as text) — download it at ${url}.`);
            text = null;
        }
    }
    // ── Truncate honestly. ──
    ...
    if (text != null && text.length > maxChars) {
        ...
        truncated = true;
        notes.push(`Text truncated to maxChars=${maxChars} characters (the document is longer). Raise maxChars or fetch the file directly for the full text.`);
    }
