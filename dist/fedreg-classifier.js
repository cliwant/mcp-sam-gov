...
const PATTERNS = [
    // ─── FAR amendment (highest specificity) ─────────────────────
    {
        ...
        field: "cfr",
        test: (i) => (i.cfrReferences ?? []).some((c) => String(c.title) === "48"),
        weight: 50,
    },
    {
        cls: "far_amendment",
    ...
    },
    {
        ...
        name: "cfr-title-13",
        field: "cfr",
        test: (i) => (i.cfrReferences ?? []).some((c) => String(c.title) === "13" && (c.part === "121" || c.part === "124" || c.part === "125" || c.part === "126" || c.part === "127" || c.part === "128")),
        weight: 30,
    },
    {
        cls: "set_aside_policy",
        name: "agency-sba",
        field: "any",
        test: (i) => (i.agencies ?? []).some((a) => /small[-\s]business|sba/i.test(a.name ?? "") || a.slug === "small-business-administration"),
        weight: 15,
    },
...
function fieldText(input, field) {
    switch (field) {
        case "title":
...
export function classifyDocument(input) {
    const scoresByClass = { ...ZERO_SCORES };
    const signals = [];
...
function buildRationale(cls, signals, input) {
...
//# sourceMappingURL=fedreg-classifier.js.map