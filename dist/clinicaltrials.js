...
export async function searchStudies(args) {
    ...
    if (typeof b.totalCount === "number" && Number.isFinite(b.totalCount)) {
        totalAvailable = num(b.totalCount); // EXACT (genuine 0 → 0)
    }
    else if (isContinuation) {
        totalAvailable = null;
    }
    else {
        throw driftError(CT_LABEL, "clinicaltrials shape drift — /studies totalCount missing/non-number on a FIRST-page countTotal=true request (typeof-checked BEFORE num(); NEVER fall back to studies.length).");
    }
    ...
    if (isContinuation && totalAvailable === null) {
        notes.push("totalAvailable is null on this continuation page — ClinicalTrials.gov reports the study total ONLY on the first page of a cursor sequence (it is omitted on every pageToken continuation, even with countTotal=true). The total from the first page still describes the whole result set.");
    }
    notes.push(...andNotes);
