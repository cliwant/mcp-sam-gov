...
export { num };
export const OPEN_CHECKBOOK_PORTALS = [
    {
        key: "sd",
...
async function getCheckbook(portal, params) {
 ...
 */
export async function openCheckbookSearch(args) {
    ...
    const filtersApplied = ["portal"];
    const params = new URLSearchParams();
    params.set("year", args.year && args.year.trim() ? args.year.trim() : "All Years");
    if (args.year && args.year.trim())
        filtersApplied.push("year");
    if (args.vendor && args.vendor.trim()) {
        params.set("vendor", args.vendor.trim());
        filtersApplied.push("vendor");
    }
    if (args.org && args.org.trim()) {
        params.set("org1", args.org.trim());
        filtersApplied.push("org");
    }
    if (args.expenseCategory && args.expenseCategory.trim()) {
        params.set("expense_category", args.expenseCategory.trim());
        filtersApplied.push("expenseCategory");
    }
    ...
    if (!Array.isArray(b.data))
        throw driftError(`open-checkbook:${portal.key}`, "Open-Checkbook shape drift — response.data must be an array.");
    const totalAvailable = num(b.count);
    if (totalAvailable === null)
        throw driftError(`open-checkbook:${portal.key}`, "Open-Checkbook shape drift — response.count must be a number.");
    ...
    return withMeta({ portal: portal.key, rows }, {
...
//# sourceMappingURL=open-checkbook.js.map