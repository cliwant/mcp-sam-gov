...
export async function captureBrief(args) {
    ...
    const agencyResult = await safe(async () => {
        ...
        const m = r.matches[0];
        if (!m) {
            throw new Error(`No USAspending agency matched "${args.agency}". Try a full name (e.g. "Department of Veterans Affairs") or abbreviation ("VA", "DHS").`);
        }
        return {
            canonical: m.name,
    ...
    const [subagencyBreakdown, topRecipients, recompetePile, recentRegulatoryActivity, activeOpportunities,] = await Promise.all([
        safe(() => usas.searchSubAgencySpending({
            agency: canonicalAgency,
            fiscalYear,
        }), "usas_search_subagency_spending"),
        safe(() => usas.searchAwards({
            agency: canonicalAgency,
            naics: args.naics,
            fiscalYear,
        }), "usas_search_awards"),
        safe(() => usas.searchExpiringContracts({
            agency: canonicalAgency,
            naics: args.naics,
        ...
        safe(async () => {
            ...
            const matched = agenciesResp.agencies.find((a) => a.name.toLowerCase().includes(agencyName) ||
        ...
        safe(() => args.sam.searchOpportunities({
            ncode: args.naics,
    ...
    const lines = [];
    if (agencyResult.ok) {
        lines.push(`Agency: ${agencyResult.data.canonical} (toptier ${agencyResult.data.toptierCode}).`);
    }
    else {
        lines.push(`Agency lookup failed: ${agencyResult.error.message}`);
    }
    if (subagencyBreakdown.ok) {
        const r = subagencyBreakdown.data;
        lines.push(`${r.results?.length ?? 0} sub-agencies with FY${fiscalYear} spending.`);
    }
    if (topRecipients.ok) {
        const r = topRecipients.data;
        lines.push(`${r.results?.length ?? 0} top recipients in NAICS ${args.naics}.`);
    }
    if (recompetePile.ok) {
        const r = recompetePile.data;
        lines.push(`${r.results?.length ?? 0} contracts expiring in next 12 months.`);
    }
    if (recentRegulatoryActivity.ok) {
        const r = recentRegulatoryActivity.data;
        lines.push(`${r.results?.length ?? 0} Federal Register documents in last 90 days.`);
    }
    if (activeOpportunities.ok) {
        const r = activeOpportunities.data;
...
export async function recompeteRadar(args) {
    ...
    const canonicalAgency = agencyResult.ok
        ? agencyResult.data.canonical
        : args.agency;
    const [expiringContracts, currentTopRecipients, rulesAffectingRecompete] = await Promise.all([
        safe(() => usas.searchExpiringContracts({
            agency: canonicalAgency,
            naics: args.naics,
            monthsUntilExpiry,
        ...
        safe(() => fedreg.searchDocuments({
            query: `recompete OR set-aside OR ${args.naics}`,
            publicationDateFrom: new Date(Date.now() - 180 * 24 * 60 * 60 * 1000)
                .toISOString()
                .slice(0, 10),
            perPage: 5,
        }), "fed_register_search_documents"),
    ...
    if (expiringContracts.ok) {
        const r = expiringContracts.data;
        lines.push(`${r.results?.length ?? 0} contracts expiring in next ${monthsUntilExpiry} months in NAICS ${args.naics}.`);
    }
    if (currentTopRecipients.ok) {
        const r = currentTopRecipients.data;
        lines.push(`${r.results?.length ?? 0} current top recipients (FY${fiscalYear}).`);
    }
    if (rulesAffectingRecompete.ok) {
        const r = rulesAffectingRecompete.data;
...
export async function vendorProfile(args) {
    ...
    const canonical = await safe(async () => {
        ...
        if (!m) {
            throw new Error(`No USAspending recipient matched "${args.recipientName}". Try a partial name or DBA.`);
        }
        return {
    ...
    const canonicalName = canonical.ok
        ...
        : args.recipientName;
    const [recipientHierarchy, recentAwards, subawardAppearances] = await Promise.all([
        safe(() => usas.searchRecipients({
            keyword: canonicalName,
            limit: 10,
        }), "usas_search_recipients"),
        safe(() => usas.searchAwardsByRecipient({
            recipientName: canonicalName,
            fiscalYear,
            limit: 20,
        }), "usas_search_awards_by_recipient"),
        safe(() => usas.searchSubawards({
    ...
    if (recentAwards.ok) {
        const r = recentAwards.data;
        lines.push(`${r.results?.length ?? 0} prime awards in FY${fiscalYear}.`);
    }
    if (subawardAppearances.ok) {
        const r = subawardAppearances.data;
        lines.push(`${r.results?.length ?? 0} prime contracts where this firm appears (own or as sub).`);
    }
    return {
