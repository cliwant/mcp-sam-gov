...
const PROVENANCE_NOTE = "Source: the CISA get.gov OFFICIAL .gov domain registry, published as CSV at github.com/cisagov/dotgov-data (the canonical location; get.gov links there). CISA is the first-party .gov registrar; this is authoritative public data served from GitHub, not a .gov API host.";
const CLIENT_FILTER_NOTE = "The registry has no query API — the full published CSV is fetched and filtered CLIENT-SIDE (organization/domain/city are case-insensitive SUBSTRING matches; state/domainType are case-insensitive). totalAvailable is the EXACT count of matching rows.";
const FRESHNESS_NOTE = "The CISA dotgov-data CSV is refreshed ~daily; this response reflects the currently-published snapshot (served from a 6-hour cache).";
const EMAIL_DROP_NOTE = "The registry's 'Security contact email' column (an organization security mailbox) is intentionally EXCLUDED — this tool resolves organizations, not contacts.";
...
async function loadRegistry(scope) {
    return memoize(`getgov:${scope}`, async () => {
        ...
        if (built.hostname !== DOTGOV_HOST || built.protocol !== "https:") {
            throw driftError(SOURCE_LABEL, `Constructed get.gov URL host ${JSON.stringify(built.hostname)} is not ${DOTGOV_HOST} over https — refusing to fetch (SSRF safety).`);
        }
        ...
        if (iDomain < 0 || iType < 0 || iOrg < 0) {
            throw driftError(SOURCE_LABEL, `get.gov registry CSV header is missing a required column (Domain name / Domain type / Organization name) — schema drift. Got: ${header.join(", ")}.`);
        }
...
export async function searchGovDomains(args) {
    ...
    if (args.domain !== undefined)
        filtersApplied.push("domain");
    if (args.domainType !== undefined)
        filtersApplied.push("domainType");
    if (args.state !== undefined)
        filtersApplied.push("state");
    if (args.city !== undefined)
        filtersApplied.push("city");
    const matched = all.filter((row) => {
        if (orgQ && !row.organization.toLowerCase().includes(orgQ))
            return false;
        if (domQ && !row.domain.toLowerCase().includes(domQ))
            return false;
        if (typeQ && !row.domainType.toLowerCase().includes(typeQ))
            return false;
        if (stateQ && (row.state ?? "").toLowerCase() !== stateQ)
            return false;
        if (cityQ && !(row.city ?? "").toLowerCase().includes(cityQ))
            return false;
        return true;
    });
    ...
    return withMeta({ scope, domains: page }, {
...
//# sourceMappingURL=gov-domains.js.map