...
export async function businessPatterns(args) {
    ...
    let body;
    try {
        body = await getJson(built.toString(), {
        ...
        if (e instanceof ToolErrorCarrier) {
            ...
            if (st !== undefined && st >= 300 && st < 400) {
                throw new ToolErrorCarrier({
                    kind: "invalid_input",
                    retryable: false,
                    message: "Census Data API redirected the request (HTTP 3xx to the 'Missing Key' page) — CENSUS_API_KEY is missing or invalid. Check the key at https://api.census.gov/data/key_signup.html.",
                    upstreamEndpoint: CENSUS_DATA_LABEL,
                });
            }
