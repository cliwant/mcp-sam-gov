...
export const GSA_CSV_URL = "https://falextracts.s3.amazonaws.com/Contract%20Opportunities/datagov/ContractOpportunitiesFullCSV.csv";
...
export function resolveCsvConfig(env = process.env) {
 ...
 */
export function parseRecordFields(text, maxCol = MAX_COL) {
    const fields = [];
    let field = "";
    let inQuotes = false;
    let col = 0;
 ...
 */
function makeRecordAssembler(onRecord) {
    let pending = null;
    return {
        push(line) {
...
async function headCsv() {
    try {
        const r = await fetchWithRetry(GSA_CSV_URL, { method: "HEAD", signal: AbortSignal.timeout(15_000) }, "gsa:csv:head");
 ...
 */
async function downloadCsvToDisk(cacheDir) {
    await mkdir(cacheDir, { recursive: true });
    const finalPath = path.join(cacheDir, RAW_CSV_FILE);
    const tmpPath = finalPath + ".tmp";
    const r = await fetchWithRetry(GSA_CSV_URL, { signal: AbortSignal.timeout(180_000) }, "gsa:csv:download");
    if (!r.body) {
    ...
    await pipeline(r.body, createWriteStream(tmpPath));
...
async function readPersistedIndex(cacheDir) {
    try {
        const raw = await readFile(path.join(cacheDir, INDEX_FILE), "utf8");
        const parsed = JSON.parse(raw);
        if (!parsed || typeof parsed !== "object" || !parsed.notices)
            return null;
        return parsed;
    }
    catch {
...
async function writePersistedIndex(cacheDir, idx) {
    await mkdir(cacheDir, { recursive: true });
...
function isStale(idx, head) {
    const ageMs = Date.now() - new Date(idx.builtAt).getTime();
    if (!Number.isFinite(ageMs) || ageMs > INDEX_TTL_MS)
        return true;
...
async function ensureIndex(cfg) {
    ...
    if (cfg.fixturePath) {
        ...
        refreshInFlight = (async () => {
            const st = await stat(cfg.fixturePath).catch(() => {
                throw new ToolErrorCarrier({
                    kind: "not_found",
            ...
            const { notices, rowCount } = await buildIndexFromFile(cfg.fixturePath);
            const persisted = {
 ...
 */
export async function lookupNoticeFields(args, env = process.env) {
    ...
    if (!cfg.enabled) {
        ...
        return withMeta({ results, enabled: false, freshness: null }, {
    ...
    try {
        ...
        throw new ToolErrorCarrier({
            kind: "upstream_unavailable",
            message: `GSA CSV backbone failed to build its index: ${e.message}. The 225 MB CSV download or parse did not complete; retry (the cache may warm on a subsequent call).`,
            retryable: true,
    ...
    let foundCount = 0;
    let missingCount = 0;
    const results = requested.map((rawId) => {
    ...
    const notes = [];
    notes.push(`Enrichment fields are read from the GSA daily bulk CSV snapshot (keyless), keyed by NoticeId. Snapshot last-modified: ${index.csvLastModified ?? "unknown"}; index built: ${index.builtAt} from ${index.rowCount.toLocaleString()} CSV rows.`);
    if (warmed) {
        notes.push("CSV index WARMING (first call this process / daily refresh): the ~226 MB CSV was just streamed to disk and indexed, so THIS call was slow (tens of seconds). Subsequent calls hit the cached on-disk index and return instantly until the daily refresh.");
    }
    notes.push("setAside is the CSV's human-readable label (e.g. 'Total Small Business Set-Aside (FAR 19.5)'); setAsideCode is the short code (e.g. 'SBA', '8A') that matches sam_get_opportunity's setAside. type is the CSV's procedure-type label (e.g. 'Solicitation', 'Combined Synopsis/Solicitation').");
    if (missingCount > 0) {
        notes.push(`${missingCount} of ${requested.length} requested noticeId(s) were NOT in the current CSV snapshot (found:false + null fields) — a snapshot is a point-in-time daily file, so a very new notice (posted after the snapshot) or one dropped from the extract will be absent. This is an explicit "not in current CSV snapshot" disclosure, NOT fabricated data; fall back to sam_get_opportunity for those noticeIds.`);
    }
    notes.push("The CSV snapshot can lag the live HAL by up to ~24 h; for a field that must be real-time (e.g. a deadline just amended), confirm with sam_get_opportunity.");
    ...
    return withMeta({ results, enabled: true, freshness, foundCount, missingCount }, {
        source: GSA_CSV_SOURCE,
        keylessMode: true,
...
//# sourceMappingURL=gsa-csv.js.map