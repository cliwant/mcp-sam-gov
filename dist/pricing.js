...
const CALC_BASE = "https://api.gsa.gov/acquisition/calc/v3/api/ceilingrates/";
async function getJson(url, headers, label, timeoutMs = 15_000) {
    const r = await fetchWithRetry(url, { headers, signal: AbortSignal.timeout(timeoutMs) }, label);
    return (await r.json());
}
 ...
 */
function coverageToIndex(coverage) {
...
function resultStateCodes(r) {
    const codes = new Set();
    for (const s of r.location?.states ?? []) {
        if (s.code)
            codes.add(String(s.code).toUpperCase());
    }
    const single = r.location?.state?.code;
    if (single)
        codes.add(String(single).toUpperCase());
    return [...codes];
}
/** Normalize the county list (name + code) from either shape. */
function resultCounties(r) {
    const out = [];
    for (const s of r.location?.states ?? []) {
        for (const c of s.counties?.include ?? []) {
            if (c.value)
                out.push({ code: String(c.code ?? ""), name: c.value });
        }
    }
...
function resultConstructionTypes(r) {
    const ct = r.constructionTypes;
    if (!ct)
        return [];
    if (Array.isArray(ct))
        return ct.map((v) => String(v).toLowerCase());
    if (typeof ct === "string")
...
export async function searchWageDeterminations(args) {
    ...
    const filtersApplied = [`coverage(${index})`];
    const filtersDropped = [];
    const notes = [];
    if (activeOnly)
        filtersApplied.push("activeOnly");
    if (standardOnly)
        filtersApplied.push("standardOnly");
    if (args.query)
        filtersApplied.push("query(WD number/title only)");
    // Build the base URLSearchParams that are shared across every page fetch.
    function buildParams(pg, sz) {
    ...
    const needsClientState = stateFilter !== undefined && !/^[A-Z]{2}$/.test(stateFilter);
    if (stateFilter) {
        if (!needsClientState) {
            ...
            notes.push(`The state value '${args.state}' is not a 2-letter USPS code; the SGS 'state' param only matches 2-letter codes (a full name returns 0), so it was applied CLIENT-SIDE over the fetched page instead. Pass a 2-letter code (e.g. 'VA') for a precise server-side filter.`);
        }
    }
    ...
    if (!needsFullScan) {
        ...
        const json = await getJson(url, SAM_HAL_HEADERS, `sam:sgs:${index}`);
        ...
        if (!needsClientState && stateFilter)
            scanParams.set("state", stateFilter);
        const firstUrl = `${SGS_BASE}?${scanParams.toString()}`;
        const firstJson = await getJson(firstUrl, SAM_HAL_HEADERS, `sam:sgs:${index}`);
        ...
        if (totalPages > WD_SCAN_PAGE_CAP)
            scanCapHit = true;
        for (let pg = 1; pg < pagesToFetch; pg++) {
            const p = buildParams(pg, WD_SCAN_SIZE);
            if (!needsClientState && stateFilter)
                p.set("state", stateFilter);
            const pgJson = await getJson(`${SGS_BASE}?${p.toString()}`, SAM_HAL_HEADERS, `sam:sgs:${index}`);
    ...
    let filtered = allRaw;
    if (needsClientState && stateFilter) {
        filtered = filtered.filter((r) => resultStateCodes(r).some((c) => c === stateFilter || r.location?.state?.name?.toUpperCase() === stateFilter));
    }
    if (countyFilter) {
        filtersApplied.push("county(client-side, full-state scan)");
        filtered = filtered.filter((r) => resultCounties(r).some((c) => c.name.toLowerCase().includes(countyFilter)));
    }
    if (ctFilter) {
        filtersApplied.push(`constructionType(client-side, value=${args.constructionType})`);
        filtered = filtered.filter((r) => resultConstructionTypes(r).includes(ctFilter));
    }
    ...
    notes.push(`Use sam_get_wage_rates with a fullReferenceNumber + revisionNumber to read the actual prevailing-wage rate table (the rates are parsed from the WD's plain-text document). ${index === "sca" ? "SCA WDs carry a WD-wide Health & Welfare rate." : "DBA WDs carry PER-CRAFT fringe rates."}`);
    notes.push("The `q` parameter matches the WD number/title only — it does NOT search by occupation or job title (e.g. q=guard returns 0). To find rates for a specific occupation, open the WD and read its rate table.");
    // ── Honest meta ──────────────────────────────────────────────────
    ...
    if (needsFullScan) {
        ...
        const capNote = scanCapHit
            ...
            : "";
        notes.push(`Scanned ${scannedCount} of ${serverTotal ?? scannedCount} ${stateLabel} ${index.toUpperCase()} WDs across all pages${capNote}. ` +
    ...
    return withMeta({ determinations, coverageIndex: index, page: needsFullScan ? 0 : page, limit: needsFullScan ? returned : limit }, {
...
async function latestActiveRevision(fullReferenceNumber) {
    const url = `${WDOL_BASE}/${encodeURIComponent(fullReferenceNumber)}/history`;
    const json = await getJson(url, SAM_HAL_HEADERS, "sam:wdol:history");
    ...
    if (active?.revisionNumber !== undefined)
        return active.revisionNumber;
...
function parseScaDocument(doc) {
    ...
    let healthAndWelfarePerHour = null;
    for (const line of lines) {
    ...
    const parseConfidence = rates.length >= 5 && healthAndWelfarePerHour !== null ? "high" : "low";
...
function parseDbaDocument(doc) {
    ...
    const eo = parseExecutiveOrderMinimum(doc);
    const parseConfidence = rates.length >= 3 ? "high" : "low";
 ...
 */
function parseExecutiveOrderMinimum(doc) {
...
export async function getWageRates(args) {
    ...
    let detail;
    try {
        detail = await getJson(url, SAM_HAL_HEADERS, "sam:wdol:detail");
    }
    catch (e) {
    ...
    if (format !== "raw") {
        if (coverage === "SCA") {
            ...
            data.rates = p.rates;
            data.executiveOrderMinimumWage = p.executiveOrderMinimum;
            notes.push("DBA (Davis-Bacon) wage determination: each rate row carries its OWN `fringePerHour` (fringe is PER-CRAFT, not a single WD-wide figure). Craft labels are parsed from a fixed-width text form and may include wrapped scope text; set format:'raw' to read the exact document.");
        }
        ...
        if (parsed && parsed.executiveOrderMinimum === null) {
            ...
            notes.push("No Executive Order minimum-wage figure could be parsed from this WD's text.");
        }
        else if (parsed && parsed.executiveOrderMinimum) {
            notes.push(`Executive-order minimum-wage FLOOR parsed from the WD text: ${parsed.executiveOrderMinimum.executiveOrder} at $${parsed.executiveOrderMinimum.minimumWage.toFixed(2)}/hour. This floor is read from whichever EO the document cites (13658 vs 14026) and applies on top of any lower listed base rate.`);
        }
        if (parseConfidence === "low") {
            notes.push("parseConfidence is LOW — the fixed-width layout did not match cleanly. Prefer the raw `document` text (call again with format:'raw' or 'both') rather than trusting the parsed rows.");
        }
    }
    ...
    if (format === "raw") {
        ...
        notes.push("format:'raw' — the full wage-determination text is returned in `document`; no rate parsing was applied.");
    }
    notes.push("Wage rates are PARSED best-effort from the WD's plain-text document (SAM does not expose them as structured JSON)." +
    ...
    return withMeta(data, {
        ...
        keylessMode: true,
        returned: Array.isArray(data.rates) ? data.rates.length : 1,
        totalAvailable: Array.isArray(data.rates) ? data.rates.length : 1,
        truncated: false,
...
export async function benchmarkLaborRates(args) {
    ...
    function buildUrl(matcher, page) {
        const p = new URLSearchParams();
        if (matcher === "search")
            p.set("search", `labor_category:${laborCategory}`);
        else
            p.set("q", laborCategory);
        p.set("page", String(page));
        p.set("page_size", String(PAGE_ROWS));
        p.set("ordering", "current_price");
        let qs = p.toString();
        for (const f of filters)
            qs += `&filter=${encodeURIComponent(f)}`;
        return `${CALC_BASE}?${qs}`;
    }
    ...
    if (args.priceRange) {
        ...
        notes.push("priceRange was NOT applied and is reported in _meta.filtersDropped: GSA CALC v3 IGNORES the price_range filter (it does not narrow the result set — live-verified), so the counts/distribution above are over the UN-narrowed set. To narrow, use businessSize / educationLevel (code) / minYearsExperience / sin, or a more specific laborCategory, instead.");
    }
    if (saturated) {
        notes.push(`matchCount is SATURATED: the API returned relation='${totalRelation}' at ${matchCount}, so the true match total is AT LEAST ${matchCount} (unknown exact). totalAvailable is null. Narrow with filters (businessSize, educationLevel code, minYearsExperience, sin) or a more specific laborCategory for an exact count.`);
    }
    if (fuzzy) {
        notes.push(`No exact labor_category match for '${laborCategory}' — FELL BACK to a loose keyword (q) match, which is broad and low-precision (q matches many unrelated categories). Treat these results as FUZZY; refine the laborCategory to a canonical CALC label for a precise band.`);
    }
    if (args.educationLevel) {
        notes.push("CALC's education_level FILTER expects short codes (e.g. BA, MA, AA, HS, PHD) — the returned education_level FIELD may show full words ('Bachelors','Masters','High School') for the same rows, so filter values differ from displayed values. If the filter did not narrow, try the short-code form. See educationLevelsInSample for the actual returned vocabulary.");
    }
    return withMeta(data, {
        source: CALC_SOURCE,
        keylessMode: true,
