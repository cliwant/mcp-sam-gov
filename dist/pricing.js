...
export async function searchWageDeterminations(args) {
    ...
    if (countyFilter) {
        ...
        notes.push("County is filtered CLIENT-SIDE over the fetched page only (the SGS API has no county filter). A county filter combined with a small limit can miss WDs on later pages — raise `limit` or narrow by `state` (server-side) to be sure.");
    }
    ...
    if (clientFiltered) {
        notes.push("totalAvailable is null because a client-side filter (county and/or a non-code state) was applied over just the fetched page — the true match count for the combined filter is unknown. The server-side total for the coverage/state/active filters was " +
            (serverTotal ?? "unknown") +
            ".");
    }
    return withMeta({ determinations, coverageIndex: index, page, limit }, {
