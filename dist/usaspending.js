 ...
 */
function awardingAgencyFilter(agency) {
    if (/^\d+$/.test(agency.trim())) {
        throw new ToolErrorCarrier({
            kind: "invalid_input",
            retryable: false,
            message: `agency ${JSON.stringify(agency)} looks like a toptier CODE, but this filter matches the canonical agency NAME (e.g. "Department of Veterans Affairs") — a code silently matches nothing here and would return a false empty. Resolve the name via usas_lookup_agency or usas_list_toptier_agencies, or use a code-based tool (usas_get_agency_profile / usas_get_agency_budget_function) that takes a toptierCode.`,
        });
    }
...
export async function searchSubawards(args) {
    ...
    const data = {
        subawards: results.map((r) => ({
            ...
            // `amount` uses below).
            subRecipient: r["Sub-Awardee Name"] ?? r["Sub-Award Recipient"] ?? null,
...
export async function getAgencyProfile(toptierCode) {
    ...
    return {
        ...
        spendingNote: "This profile has NO obligation/spending figure. For this agency's obligations use: usas_get_agency_awards_summary(toptierCode) = AWARD-level obligations for a fiscal year; usas_list_toptier_agencies → this agency's `obligatedAmount` = ACCOUNT-level total (all spending, higher than award-level). For CONTRACTS-only, usas_spending_over_time / usas_search_*_spending by the agency's canonical name.",
    };
}
...
export async function listDisasterCodes() {
    return memoize("usas:def_codes", async () => {
        const json = await getUsas("references/def_codes/");
        ...
        return withMeta({ codes }, referenceMeta({
...
export async function disasterSpending(args) {
    ...
    const json = await postUsas("disaster/spending_by_geography/", {
    ...
    return withMeta({ spendingType, geoLayer, defCodes: args.defCodes, geographies }, {
