 ...
 */
function parseSnapshotEnvelope(raw, label) {
    if (typeof raw !== "object" || raw === null) {
        throw driftError(label, `Snapshot ${label} is not a JSON object.`);
    }
    const env = raw;
    if (typeof env.asOf !== "string" || env.asOf.length === 0) {
        throw driftError(label, `Snapshot ${label} is missing a string 'asOf' (P5 freshness is mandatory for a non-live body).`);
    }
    if (!("data" in env)) {
    ...
    if (env.accessLevel !== undefined && env.accessLevel !== "public") {
        throw driftError(label, `Snapshot ${label} accessLevel is ${JSON.stringify(env.accessLevel)}, not "public" — refusing to serve (public-only policy).`);
    }
    return { data: env.data, asOf: env.asOf };
}
/**
...
export function snapshotPath(key) {
    ...
    if (base === undefined)
        return null; // INERT: snapshot disabled ⇒ no path.
    if (!SNAPSHOT_KEY_RE.test(key)) {
        ...
        throw driftError(`snapshot:${key}`, `Invalid snapshot key ${JSON.stringify(key)} (must match ${SNAPSHOT_KEY_RE}).`);
    }
    ...
    const provenance = { dataPath: "snapshot" };
    return {
        host,
        provenance,
        run: async () => {
            ...
            const raw = await getJson(url, { label, redirect: "error" });
            const { data, asOf } = parseSnapshotEnvelope(raw, label);
