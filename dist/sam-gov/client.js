...
const DEFAULT_USER_AGENT = "Mozilla/5.0 (compatible; @cliwant/mcp-sam-gov; +https://github.com/cliwant/mcp-sam-gov)";
export class SamGovClient {
                ...
                if (r.ok) {
                    ...
                    if (hit) {
                        if (!hit.resourceLinks || hit.resourceLinks.length === 0) {
                            ...
                            hit.resourceLinks = await this.getPublicResourceLinks(noticeId).catch(() => {
    ...
    async fetchOpportunityDescription(input) {
        ...
        try {
            ...
            if (ct.includes("application/json") || ct.includes("application/hal+json")) {
                const json = (await r.json());
                return ((json.body ?? json.data?.body ?? json.description ?? "").trim() ||
    ...
    async searchEntities(query) {
        ...
        try {
            ...
            if (!r.ok)
                throw new Error(`Entity search ${r.status}`);
            const json = (await r.json());
            return {
                entities: (json.entityData ?? []).map((e) => ({
                    ...
                    naics: e.assertions?.goodsAndServices?.naicsList?.map((n) => n.naicsCode ?? "") ?? [],
                    activeRegistration: e.entityRegistration?.registrationStatus === "Active",
                })),
    ...
    buildAuthSearchUrl(filters) {
        ...
        if (filters.query)
            url.searchParams.set("title", filters.query);
        if (filters.ptype?.length)
            url.searchParams.set("ptype", filters.ptype.join(","));
        if (filters.ncode)
            url.searchParams.set("ncode", filters.ncode);
     ...
     */
    async getPublicResourceLinks(noticeId) {
        ...
        if (!r.ok)
            throw new Error(`resources HTTP ${r.status}`);
        const json = (await r.json());
        const attachments = json._embedded?.opportunityAttachmentList?.[0]?.attachments ?? [];
        return attachments
