...
export function escapeODataString(v) {
    return v.replace(/'/g, "''");
}
 ...
 */
export function buildFilter(datasetKey, clauses) {
    if (clauses.length === 0)
        return null;
 ...
 */
async function getOpenFema(datasetKey, params) {
    ...
    try {
        ...
        if (e instanceof SyntaxError) {
            throw driftError(SOURCE_LABEL, "OpenFEMA returned non-JSON at HTTP 200 — possible maintenance page");
        }
        throw e;
    }
...
// ─── map + meta ───────────────────────────────────────────────────
const SHAPE_NOTE = "OpenFEMA OData envelope: rows live under the entity key (e.g. body.DisasterDeclarationsSummaries) and totalAvailable is the EXACT filtered total from metadata.count (we always send $inlinecount=allpages). Never inferred from the page length.";
const AMOUNT_NOTE = "Amount fields (projectAmount / federalShareObligated / totalObligated / mitigationAmount) are number|null — a real 0 stays 0; absent/empty is null (never 0).";
const DATE_NOTE = "Date filters use OData ISO compares on declarationDate; a bare 'YYYY-MM-DD' means midnight-UTC start of that day (pass a full ISO datetime for finer bounds; a 'to' bound is exclusive of intra-day times after midnight).";
/**
 ...
 */
function coerceAmounts(rows, amountFields) {
...
function shapeResponse(args) {
    ...
    if (!Array.isArray(rawRows)) {
        throw driftError(SOURCE_LABEL, `OpenFEMA ${def.entityName} returned an unexpected shape (body[${JSON.stringify(def.entityName)}] must be an array of rows).`);
    }
    ...
    if (typeof b.metadata?.count !== "number") {
        throw driftError(SOURCE_LABEL, `OpenFEMA ${def.entityName} returned a non-number metadata.count — with $inlinecount=allpages it must be the exact filtered total; treating as schema drift.`);
    }
    ...
    if (def.amountFields.length > 0)
        notes.push(AMOUNT_NOTE);
    if (args.filtersApplied.some((f) => /date/i.test(f)))
        notes.push(DATE_NOTE);
    ...
    if (args.offset > 100000) {
        notes.push("Deep offset (>100000): very deep $skip into a large dataset may degrade upstream; prefer narrowing the filter over paging deep.");
    }
    return withMeta({
