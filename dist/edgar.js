 ...
 */
function fuzzyResolveNote(resolved, input) {
    if (resolved.matchedBy !== "title")
        return null;
    return `Resolved "${input}" to ${resolved.title ?? "this filer"} (CIK ${resolved.cik}) by a case-insensitive TITLE SUBSTRING match, NOT an exact ticker/CIK — a substring can silently match the WRONG company. VERIFY this is the intended filer (use edgar_lookup_cik to list all matches, or pass an exact ticker or 10-digit CIK).`;
}
// ─── meta helper ──────────────────────────────────────────────────
/**
...
export async function companyFacts(args) {
    ...
    notes.push("Each point carries `start`: null for INSTANT (balance-sheet) concepts and the ISO period-start for DURATION (flow) concepts. Period identity is the (start,end) PAIR — a same-`end` different-`start` pair is a different-duration fact (a 3-month quarter vs the 12-month year), NOT a revision.");
    if (sameEndMultiDuration.length) {
        notes.push(`Concept(s) with MULTIPLE durations sharing the same period-end (a shorter quarter AND a longer YTD/annual figure coexist under one \`end\`): ${sameEndMultiDuration.join(", ")}. Each point's \`start\` distinguishes them — do NOT compare or sum values across differing \`start\` (e.g. never read a Q4 figure as the full year) without checking \`start\`.`);
    }
    notes.push(latest
