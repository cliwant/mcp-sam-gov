...
### Changed
- **Trim tool descriptions to cut tools/list token cost.** Rewrote 41 tool descriptions and 2 parameter descriptions to enforce <=1600-char tool description and <=400-char param description limits. Grand total compact-JSON chars reduced from 297,230 (~74,308 tokens) to 273,532 (~68,383 tokens). A new lint-invariants.mjs check (4) enforces these limits in CI so regressions turn CI red.
