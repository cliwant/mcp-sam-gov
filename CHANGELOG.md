...
### Changed
- **Anthropic MCP Directory listing requirements.** The MCPB bundle now ships a 512×512 `icon.png` referenced by the manifest's `icon` field, and the manifest description, the long description and all three READMEs (EN/KO/JA) state plainly that this is an independent project — not affiliated with, endorsed by, or sponsored by SAM.gov, the U.S. General Services Administration, or any government agency.
