...
- **State & local data map added to `skills/sam-gov/SKILL.md`.** New `## State & local data map` section: a jurisdiction-by-jurisdiction lookup table of verified dataset IDs, row counts, and "NOT" caveats for 11 state-level open-data sources (VA eVA 1.69M rows via `ckan_query`; MA CTHRU ~49M via `socrata_query`; NJ 660k; NY authority contracts 275k + MTA 107k; WA agency contracts 79k + master-contract sales 246k; MT contracts ~4.5k via `tableau_view_csv`; SD checkbook ~741k via `open_checkbook_search`; IL CDB bids 48 rows; IL IDHR vendor directory 7.8k) plus 10 county/city Socrata hosts and the 2 new ArcGIS county CIP pipeline layers (Hennepin MN, Charlotte-Mecklenburg NC). Also lists state portals not reachable keyless (CA, TX, OH, NC, MI, IL/MA/NJ Periscope). `README.md` adds a companion `> State open-data map` note in the SLED tool-catalog section linking to the skill file and citing the five largest datasets. `README.ko.md` and `README.ja.md` each add a one-line pointer in the tool-catalog SLED listing. No tool, schema, or behavior changes; docs only.
