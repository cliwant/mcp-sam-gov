...
| 5+ separate vendor APIs / scrapers | 1 unified surface, 153 tools across 54 sources |
...
This installs the MCP server **plus** a [SKILL.md](./skills/sam-gov/SKILL.md) workflow guide that teaches Claude when + how to use each of the 153 tools.
...
Restart Claude Desktop fully (system tray quit on Windows / Quit menu on macOS), then look for the 🔨 icon. You should see "sam-gov (153 tools)".
...
By default all 153 tools load — about 74k tokens per session. Clients that pay the full context cost (Claude Desktop, Cursor, raw API) can cut that significantly by loading only the toolsets relevant to the task.
...
| `vetting` | 17 | ~10k | Partner due-diligence: FAC, FDIC, EPA ECHO/TRI, CourtListener, nonprofit (IRS 990), Senate LDA lobbying |
...
| `health` | 17 | ~12k | CMS, NPPES, NIH, NSF, ClinicalTrials.gov, openFDA |
...
| **all** | **153** | **~74k** | Everything (default) |
...
<details>
<summary><b>Litigation & courts — CourtListener (2 tools)</b></summary>

- `courtlistener_search_opinions` — US federal court opinions (COFC contract claims / bid protests, Federal Circuit); `party` param for actual-party cases — data via CourtListener / Free Law Project (disclosed)
- `courtlistener_search_dockets` — RECAP dockets (type=r) for FCA / False Claims Act / qui tam matters that rarely produce opinions; suitNature fielded filter — data via CourtListener / Free Law Project (disclosed)
</details>
...
- `socrata_query` — query rows from an allowlisted Socrata/SODA open-data portal (**54 curated state/county/city + federal hosts** — checkbook, contracts, vendor payments, live solicitations across the SLED tier)
...
- `open_checkbook_search` — row-level vendor-payment search over a curated **Socrata Open Expenditures** checkbook portal (keyless; exact-match year/vendor/department/expense-category filters); two portals today: **South Dakota** (~740,980 payments, only the ~3 most recent fiscal years) and **Alaska** (41,751 payments, **FY2026 only** — any other year returns count:0, meaning "not published", not zero spending); freshness is set by the publisher and can lag, so sort by payment_date desc to see the newest date)
</details>

> **State open-data map** — also available as an MCP resource (`samgov://data-map/state-local`, readable by any MCP client via `resources/read`) and in `skills/sam-gov/SKILL.md`. Largest verified keyless datasets: Texas DIR cooperative purchase lines 10.7M rows + TxDOT live lettings 412 projects (`socrata_query` data.texas.gov), Massachusetts CTHRU vendor payments ~49M rows, Virginia eVA PO line items 1.69M rows (`ckan_query` data.virginia.gov), New Jersey YourMoney 660k rows, Washington contracts + master-contract-sales 325k rows combined. **These are state open-data mirrors, not the states' live bid portals** (TX TxDOT lettings being the first live solicitation feed). CA, OH, NC, MI, and non-TxDOT TX portals are login/WAF-gated; FL and GA have no state open-data portals (NXDOMAIN); PA's portal carries no procurement datasets.
