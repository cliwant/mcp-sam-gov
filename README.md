...
> **State open-data map** — `skills/sam-gov/SKILL.md` contains a jurisdiction-by-jurisdiction lookup of verified dataset IDs, row counts, and caveats for state and local sources. Largest verified keyless datasets: Virginia eVA PO line items 1.69M rows (`ckan_query` data.virginia.gov), Massachusetts CTHRU vendor payments ~49M rows, New Jersey YourMoney 660k rows, New York state-authority contracts 275k rows, and Washington contracts + master-contract-sales 325k rows combined. **These are state open-data mirrors, not the states' live bid portals.** Several official state portals — CA (Cal eProcure), TX, OH, NC, MI, and the Periscope-based IL/MA/NJ portals — are login- or WAF-gated and therefore not reachable keyless.
