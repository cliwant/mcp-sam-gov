...
| 🏙 **주·지방정부 조달 입찰 (SLED)** | "이 시·카운티 조달 포털에 지금 열려 있는 입찰은? 체크북·낙찰 계약·벤더 지급 내역, 주 교통국(DOT) 입찰·낙찰 대장까지" | OpenGov Procurement (525곳+ 정부), Bonfire, ArcGIS (feature + Hub), Socrata (54개 호스트), Tableau Server 게스트 뷰 1개 (몬태나주), Socrata Open Expenditures 체크북 1개 (사우스다코타주) |
| 🎓 **grant·데이터셋** | "최근 30일 사이버보안 grant, 연방 오픈 데이터셋 발굴" | Grants.gov, data.gov |

**54개 데이터 소스, 총 153개 도구 — keyless 우선: Census business-patterns · FRED · BEA · DOL 데이터 엔드포인트 4개 소스만 무료 키가 필요하고 나머지 50개 소스는 키가 필요 없습니다.** (초기 52-도구 빌드 기준 대략 p50 ~0.25s / p95 ~0.8s 측정 — 소스·업스트림 부하에 따라 변동하는 근사치이며 보장값이 아님.)
...
MCP 서버 + Claude 가 153개 도구를 언제 / 어떻게 호출할지 가르치는 [SKILL.md 워크플로 가이드](./skills/sam-gov/SKILL.md) 동시 등록.
...
기본값은 전체 153개 도구를 로드하며 세션당 약 80k 토큰이 소요됩니다. `MCP_SAM_GOV_TOOLSETS` 환경 변수에 필요한 도구셋 이름을 쉼표로 구분해 지정하면 컨텍스트를 줄일 수 있습니다. 빈 값이나 `all`로 설정하면 전체 도구가 로드됩니다. 사용 가능한 도구셋: `core`(58개, SAM/Grants/USAspending/FAR/eCFR/Federal Register/SBA 등), `sled`(13개, 주·지자체 조달), `vetting`(17개, CMS/NIH/NSF/openFDA 등), `safety`(3개, NHTSA/CPSC), `geo`(9개, Census geocode/FEMA/NWS 등), `cyber`(3개, NVD/CISA KEV/NIST 800-53). 로드되지 않은 도구를 호출하면 어느 도구셋에서 활성화할 수 있는지를 알려주는 `tool_not_loaded` 오류가 반환됩니다.
...
- **업데이트:** `npm i -g @cliwant/mcp-sam-gov@latest` — 또는 `npx @cliwant/mcp-sam-gov@latest` 설정이면 재시작만.
- **끄기:** `MCP_SAM_GOV_NO_UPDATE_CHECK=1` (또는 `NO_UPDATE_NOTIFIER=1`).
- **변경 내역:** [releases](https://github.com/cliwant/mcp-sam-gov/releases) + [CHANGELOG](./CHANGELOG.md).

## 버그 신고 / 기능 요청 — 한 번에

이 서버는 **실제 사용에서 개선**되도록 설계됐습니다. 툴을 호출하는 건 사람이 아니라 AI 에이전트이므로, 피드백도 **에이전트를 통해** 이뤄집니다:

- `schema_drift`(연방 API 형태 변경) · `upstream_unavailable`(장애) 에러 봉투에는 **`report`** 필드(미리 채운 GitHub 이슈 링크)가 실립니다.
- 또는 에이전트에게 "버그 신고" / "결과가 이상해" / "이 기능이 있으면 좋겠어"라고 하면 **`feedback` 도구**가 미리 채운 이슈 링크를 돌려줍니다.
...
워크플로별 그룹. keyless 우선 — 대부분 키가 필요 없고, Census business-patterns · FRED · BEA · DOL 데이터 엔드포인트는 무료 키가 필요합니다. 몇몇 다른 소스는 선택적 무료 키를 넣으면 한도가 올라갑니다(위 키 섹션 참고). 전체 per-tool 목록과 입력 schema·정직성 caveat 원문은 [영문 README 의 카탈로그 섹션](./README.md#tool-catalog-153-tools)을 기준으로 하세요.
...
- **소송·법원 — CourtListener (2)**: `courtlistener_search_opinions`(미 연방 법원 판례; `party` 파라미터로 실제 당사자 사건 검색 — CourtListener/Free Law Project 경유) + `courtlistener_search_dockets`(RECAP 도켓 — FCA/False Claims Act/qui tam 사건용; suitNature 필드 필터; CourtListener 경유)
...
- **규정·입법 — Federal Register · Regulations.gov · eCFR · FAR · Congress · GovInfo (19)**: `fed_register_search_documents` `fed_register_get_document` `fed_register_public_inspection` `fed_register_list_agencies` `regulations_search_dockets` `regulations_search_documents` `regulations_search_comments` `regulations_get_docket` `ecfr_search` `ecfr_get_section`(CFR 조항 하나의 전문을 인용번호로 조회 — 모든 title 대상, FAR/DFARS는 `far_clause_lookup` 권장) `ecfr_list_titles` `far_clause_lookup` `far_search` `far_compliance_matrix` `congress_search_bills` `congress_get_bill` `govinfo_search_packages` `govinfo_get_package` `govinfo_list_collections`
...
- **주·지방정부 조달 입찰 (SLED) — OpenGov · Bonfire · ArcGIS · Tableau · Socrata Open Checkbook (8)**: `opengov_list_governments`(OpenGov Procurement를 쓰는 미국 주·지방정부 525곳+ 디렉터리 — 주·이름으로 필터) `opengov_search_solicitations`(정부별 공개 솔리시테이션 — 제목·상태[open = 현재 접수 중]·마감일·포털 링크; 포털이 쓰는 keyless 익명 엔드포인트) `bonfire_list_organizations`(Bonfire(Euna)를 쓰는 미국 정부 디렉터리 — 실제 동작을 확인한 큐레이션 시드 187개, 전체 목록은 아님) `bonfire_search_opportunities`(기관별 현재 공고 중인 입찰 — keyless RSS가 공고 전체라 총계가 정확) `arcgis_hub_discover_datasets`(ArcGIS Hub 데이터셋 키워드 발굴 — Socrata·CKAN이 못 덮는 SLED GIS·인프라·인허가·조달 레이어; 게시자를 직접 검증하도록 돕는 발굴용 도구) `arcgis_feature_query`(큐레이션 허용목록 **29개 미국 정부 ArcGIS REST feature 레이어** 행 조회 — DC OCP PASS 실시간 솔리시테이션, 지자체 체크북·계약, **카운티 CIP 파이프라인(Hennepin County MN·Charlotte-Mecklenburg NC — 입찰 대장이 아닌 자본개선 사업 계획)**, **주 DOT 입찰·낙찰 대장(TX / AK / IA / OK)**, **노스다코타주 DOT 연방 flex 자금 교부 레이어 4개** — 카운티·타운십·시 같은 지방 공공기관에 배분한 교부 내역이지 벤더와 맺은 계약이나 입찰 결과가 아님. ND 주 체크북·조달 포털에 키 없이 닿을 수 없어 대신 쓰는 대리 데이터) `tableau_view_csv`(미국 정부 Tableau Server 게스트 뷰의 CSV 전체 내보내기를 키 없이 받아 페이지로 나눠 조회 — 지금은 뷰 1개: 몬태나주 행정부(DOA) Contracts Awarded 약 4,554건. 갱신 시점은 게시 기관이 정하므로 늦을 수 있음) `open_checkbook_search`(Socrata Open Expenditures 체크북 포털의 벤더 지급 건별 검색 — keyless, 연도·벤더·부서·지출 분류 정확 일치 필터. 지금은 포털 2개: 사우스다코타주 약 740,980건(최근 약 3개 회계연도)과 알래스카주 41,751건(**FY2026만** — 다른 연도의 0건은 "미공개"라는 뜻이지 지출이 0이라는 뜻이 아님). 갱신 시점은 게시 기관이 정하므로 늦을 수 있으니 최신 날짜는 payment_date 내림차순 정렬로 확인)
- **주·지방 오픈데이터 지도**: MCP 리소스 `samgov://data-map/state-local`(`resources/read`로 모든 클라이언트에서 조회 가능)과 `skills/sam-gov/SKILL.md`에 주별 검증된 데이터셋 ID·행 수·주의사항이 담긴 조회 표가 있습니다(TX DIR 구매 1,075만 건+TxDOT 현행 발주 412건·MA CTHRU ~4,900만 건·VA eVA 169만 건 등). 이들은 각 주의 공식 입찰 포털이 아닌 공개 데이터 미러이며(TX TxDOT 발주가 최초의 실시간 주 솔리시테이션 피드), FL·GA는 오픈데이터 포털 자체가 없고 PA는 조달 데이터가 없는 측정된 부재 상태입니다.
...
- **서버 유틸리티 — 키 발견 · 피드백 (2)**: `api_key_status`(각 소스에 필요한 키·required/optional·발급 URL·현재 설정 여부 나열; 값은 노출 안 함) `feedback`(버그 신고·기능 요청·이상해 보이는 결과를 알릴 때 미리 채운 GitHub 이슈 링크를 돌려줌 — 서버는 아무것도 직접 올리지 않고, 링크를 열어 제출하는 건 사용자)
