...
- **州・地方オープンデータマップ**: `skills/sam-gov/SKILL.md` に州別の検証済みデータセット ID・行数・注意事項の参照表があります(VA eVA 169万件・MA CTHRU ~4,900万件・NJ 66万件・NY 28万件・WA 32万件など)。これらは各州の公式入札ポータルではなくオープンデータミラーです。CA・TX・OH・NC・MI 等の公式ポータルはログイン・WAF ゲートのためキーレスでは到達できません。
