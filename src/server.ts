...
const NaicsRevisionCheckInput = z.object({
  naicsCode: z
    .string()
    .describe(
      "6-digit NAICS code to verify. Returns validity in NAICS 2022 + any historical change (renumbered / split / retired).",
    ),
});
...
export const TOOLS: ToolDef[] = [
  ...
  },
  {
    ...
    description:
      "Spending broken down by Product Service Code (PSC). Use for CONTRACT market structure ('what PSC categories see the most $ at DoD?') — surfaces market segments beyond NAICS (e.g. PSC R425 = engineering support services, PSC D316 = IT-end user, PSC D318 = IT-data center). Sibling tool routing: NAICS-style market = this tool; geography = usas_search_state_spending; budget account = usas_search_federal_account_spending; buyer = usas_search_agency_spending; grants (not contracts) = usas_search_cfda_spending.",
    inputSchema: UsasCategorySpendingInput,
  },
  {
    name: "usas_search_state_spending",
    description:
      "Spending broken down by state / territory. Use for GEOGRAPHIC analysis ('where is federal $ flowing for NAICS 541512?') — typical answer pattern: 'VA $128B, MD $66B, DC $58B'. Sibling routing: state geography = this tool; market structure = usas_search_psc_spending; budget account = usas_search_federal_account_spending.",
    inputSchema: UsasCategorySpendingInput,
  },
  {
    name: "usas_search_cfda_spending",
    description:
      "Spending broken down by CFDA grant program code. FOR GRANTS ONLY — CFDA = financial assistance (award_type 02-05), NOT contracts. Use this for 'top federal grant programs by $'. For contract market analysis, switch to usas_search_psc_spending. For grant OPPORTUNITIES (not historical spending), use grants_search.",
    inputSchema: UsasCfdaInput,
  },
  {
    name: "usas_search_federal_account_spending",
    description:
      "Spending broken down by federal account / Treasury Account Symbol (TAS). Use to map money to the actual BUDGET LINE ITEM (e.g. '036-0167 = Information Technology Systems, VA'). For BUDGET / appropriations questions ('which TAS funds X?'). For market segments use usas_search_psc_spending; for buyer breakdown use usas_search_agency_spending.",
    inputSchema: UsasCategorySpendingInput,
  },
  {
    name: "usas_search_agency_spending",
    description:
      "Spending broken down by AWARDING AGENCY. Use for 'which agencies spend the most on NAICS 541512?' — top buyers by $. Sibling routing: BUYER = this tool; sub-agency / office breakdown = usas_search_subagency_spending; agency PROFILE / metadata = usas_get_agency_profile.",
  ...
  // ━━━ USAspending — Recipient Profile (2) ━━━
  {
    ...
    description:
      "Search USAspending recipient list with FULL parent/child/recipient hierarchy. Returns recipients with id, duns, uei, level (P=parent, C=child, R=recipient), total_amount. Use to FIND the recipient_id (e.g. 'find Booz Allen's parent recipient_id') before calling usas_get_recipient_profile. For QUICK fuzzy name resolution (without hierarchy), prefer usas_autocomplete_recipient — it's faster and works for anti-hallucination prep.",
  ...
  // ━━━ USAspending — Reference / Autocomplete (4) ━━━
  {
    ...
    description:
      "Autocomplete NAICS codes by free-text. ANTI-HALLUCINATION GUARD — call this when the user mentions a NAICS theme but no specific code (e.g. 'computer systems design' → 541512, 'cloud computing' → 541512 or 518210). Returns top fuzzy matches with code + title. Use BEFORE any search tool that takes a NAICS code. For navigating PARENT/CHILD relationships (e.g. 'show all 6-digit NAICS under 5415'), use usas_naics_hierarchy instead.",
    inputSchema: UsasAutocompleteInput,
  },
  {
    name: "usas_autocomplete_recipient",
    description:
      "Autocomplete recipient names — FAST fuzzy lookup. ANTI-HALLUCINATION GUARD — call this BEFORE any tool that filters by recipient name to confirm the exact USAspending-canonical legal name (e.g. 'Booz Allen' → 'BOOZ ALLEN HAMILTON INC'). Returns up to 10 matches with UEI/DUNS. For full hierarchy + total spend (parent vs child relationships), use usas_search_recipients instead.",
    inputSchema: UsasAutocompleteInput,
  },
  {
    name: "usas_naics_hierarchy",
    description:
      "Navigate the NAICS hierarchy tree (2-digit → 3-digit → 4-digit → 6-digit). Returns parent/child relationships + active-contract count per code. Use for MARKET SCOPE exploration ('what 6-digit NAICS exist under 5415?', 'how many active contracts under NAICS 541?'). For free-text → code resolution, use usas_autocomplete_naics instead.",
  ...
  // ━━━ NAICS revision crosswalk (1) ━━━
  {
    ...
    description:
      "Check whether a NAICS code is valid in NAICS 2022 and surface any historical change (2002 → 2007 → 2012 → 2017 → 2022 revisions). Returns validity flag + status (stable / renumbered / split / retired) + canonical 2022 successor if changed. Catches old codes still cited in legacy SOWs (e.g. 541510 retired in 2007, 511210 renumbered to 513210 in 2022, 519130 split in 2022). Use BEFORE running USAspending or SAM.gov searches with any code from a pre-2022 contract document. Coverage v0.5: ~60 federal-contracting-relevant codes; falls back to Census concordance hint for unknown codes. For free-text → code resolution, use usas_autocomplete_naics instead.",
