  ...
  switch (name) {
    ...
    case "sam_search_opportunities": {
      ...
      if (sam.isKeyless) {
        ...
        const droppable: Array<[string, boolean]> = [
        ...
        if (queryUnverified) {
          notes.push(
            "The free-text `query` filter is not verified in keyless mode — it may only partially constrain results; do not assume every notice matches the query.",
          );
        }
