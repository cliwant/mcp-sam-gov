...
function load(): CrosswalkFile {
  ...
  for (const p of candidates) {
    try {
      ...
      cached = JSON.parse(text) as CrosswalkFile;
      return cached;
    } catch {
      // try next
    }
  ...
  throw new Error(
    "naics-revision-changes.json not found. Expected at src/data/ or one level up from dist/.",
  );
}
 ...
 */
export function checkNaicsRevision(naicsCode: string): NaicsRevisionCheck {
  const file = load();
 ...
 */
export function listStableCodes(): string[] {
 ...
 */
export function listKnownChanges(): Array<{
  code: string;
  changeType: string;
  note: string;
}> {
