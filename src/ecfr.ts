...
async function fetchXmlText(url: string): Promise<string> {
  const r = await fetchWithRetry(
    url,
    { headers: { Accept: "application/xml" }, signal: AbortSignal.timeout(20_000) },
    `ecfr:${url.split("/api/")[1] ?? url}`,
  );
  return await r.text();
}
...
export async function getSection(args: {
  ...
  if (!issueDate) {
    ...
    const t = (await listTitles()).data.titles.find(
      (x) => x.number === args.titleNumber,
    );
  ...
  const div = xml.match(
    new RegExp(`<DIV8 N="${escaped}"[^>]*TYPE="SECTION"[^>]*>([\\s\\S]*?)</DIV8>`, "i"),
  );
  if (!div) {
    throw new ToolErrorCarrier({
      kind: "not_found",
  ...
  let alternateReference: string | null = null;
  const meta = xml.match(/hierarchy_metadata="([^"]*)"/i);
  if (meta) {
    try {
      const hm = JSON.parse(decodeXmlEntities(meta[1]!)) as {
        citation?: string;
  ...
  if (fullText.length === 0) {
    ...
    throw driftError(
      "ecfr:versioner/full",
      `The section ${args.section} DIV8 carried no extractable text — eCFR XML shape drift, never a fabricated empty section.`,
    );
  }
