...
const NPPES_STATES_SET: ReadonlySet<string> = new Set(NPPES_STATES);
...
export const NPPES_REACH_CAP_NOTE =
...
function lowerBoundNote(atLeast: number): string {
...
export type NppesProvider = {
  number: string | null;
  enumerationType: string | null; // "NPI-1" | "NPI-2"
  active: boolean; // basic.status === "A"
  status: string | null; // verbatim
  basic: {
  ...
  identifiers: unknown[]; // verbatim (never flattened)
  otherNames: unknown[]; // verbatim
  endpoints: unknown[]; // ★ S1 — surfaced verbatim
...
function mapAddress(raw: unknown): NppesAddress {
  const a = rec(raw);
  return {
...
function mapTaxonomy(raw: unknown): NppesTaxonomy {
  const t = rec(raw);
  return {
    code: str(t.code),
    desc: str(t.desc),
...
function mapArray(raw: unknown, fn: (x: unknown) => NppesAddress): NppesAddress[] {
  if (!Array.isArray(raw)) return [];
  return raw.map(fn);
}

/** Verbatim array passthrough (identifiers/otherNames/endpoints — never flattened). */
function verbatimArray(raw: unknown): unknown[] {
 ...
 */
function mapProvider(raw: unknown): NppesProvider {
  const r = rec(raw);
  ...
  return {
    ...
    basic: {
      ...
      authorizedOfficialTitleOrPosition: str(
        basic.authorized_official_title_or_position,
      ),
      authorizedOfficialTelephoneNumber: str(
 ...
 */
export function cmsLuhnValid(npi: string): boolean {
  if (!NPI_RE.test(npi)) return false;
...
function sanitizeText(v: string): string {
  ...
  for (const ch of v) {
    ...
    if (code < 0x20 || code === 0x7f) continue; // drop C0 control chars + DEL
    out += ch;
  }
  return out.trim();
}
 ...
 * trailing wildcard).
 */
function checkWildcard(field: string, v: string): void {
  if (!v.includes("*")) return;
  if (v.indexOf("*") !== v.length - 1 || v.indexOf("*") !== v.lastIndexOf("*")) {
    throw invalidInput(
      `Invalid wildcard in ${field} ${JSON.stringify(v)} — NPPES supports only a SINGLE TRAILING '*' wildcard.`,
    );
  }
  const literal = v.slice(0, -1);
  if (literal.length < 2) {
    throw invalidInput(
      `Invalid wildcard in ${field} ${JSON.stringify(v)} — a trailing '*' requires at least 2 leading literal characters (NPPES rejects a single-leading-char wildcard).`,
    );
  }
}
 ...
 */
async function nppesGet(params: URLSearchParams): Promise<unknown> {
  const built = new URL(`${NPPES_URL}?${params.toString()}`);
  if (built.hostname !== NPPES_HOST || built.protocol !== "https:") {
    throw invalidInput(
      `Constructed NPPES URL host ${JSON.stringify(built.hostname)} (${built.protocol}) is not ${NPPES_HOST} over https — refusing to fetch (SSRF safety).`,
    );
  }
  ...
  try {
    return await throughGate("nppes", NPPES_GATE_MIN_INTERVAL_MS, () =>
      getJson(built.toString(), { label: NPPES_LABEL, redirect: "error" }),
    );
  } catch (e) {
    ...
    if (e instanceof SyntaxError)
      throw driftError(
        NPPES_LABEL,
        "NPPES returned a non-JSON body at HTTP 200 — schema drift.",
      );
    throw e;
  }
}

// ─── Response envelope parse (loud-fail + parity guard) ──────────
type ParsedNppes = { resultCount: number; results: unknown[] };
 ...
 */
function parseNppesBody(body: unknown): ParsedNppes {
  if (body === null || typeof body !== "object" || Array.isArray(body)) {
    throw driftError(
      NPPES_LABEL,
      "NPPES returned a 200 body that is not an object {result_count, results} — refusing to report it as an empty result.",
    );
  }
  ...
  if (Array.isArray(errors) && errors.length > 0) {
    ...
    throw invalidInput(
      `NPPES rejected the request [Errors]: ${desc}${field ? ` (field: ${field})` : ""}. (A 200 body carrying Errors and NO results key is a body-level loud-fail — never read as an empty result.)`,
    );
  }
  if (!Array.isArray(b.results)) {
    throw driftError(
      NPPES_LABEL,
      "NPPES 200 body is missing the `results` array with no `Errors` — nothing trustworthy to report (treating as schema drift, never a fake empty).",
    );
  }
  ...
  if (typeof rc !== "number" || !Number.isFinite(rc)) {
    throw driftError(
      NPPES_LABEL,
      "NPPES `result_count` absent/non-numeric — cannot report a trustworthy count (schema drift). typeof-checked BEFORE any coercion.",
    );
  }
  if (rc !== results.length) {
    throw driftError(
      NPPES_LABEL,
      `NPPES result_count (${rc}) !== results.length (${results.length}) — the count-parity guard tripped (never present a page count that disagrees with the rows).`,
    );
  }
...
function normCmp(v: string): { target: string; wild: boolean } {
  const t = v.trim();
  const wild = t.endsWith("*");
  return { target: (wild ? t.slice(0, -1) : t).toUpperCase(), wild };
}

function scalarMatches(candidate: string | null, target: string, wild: boolean): boolean {
 ...
 */
function fieldMatches(field: string, value: string, p: NppesProvider): boolean {
  const { target, wild } = normCmp(value);
  const allAddr = [...p.addresses, ...p.practiceLocations];
  switch (field) {
    case "first_name":
      return (
        scalarMatches(p.basic.firstName, target, wild) ||
        scalarMatches(p.basic.authorizedOfficialFirstName, target, wild)
      );
    case "last_name":
      return (
        scalarMatches(p.basic.lastName, target, wild) ||
    ...
    case "state":
      return (
        allAddr.some((a) => scalarMatches(a.state, target, wild)) ||
        p.taxonomies.some((t) => scalarMatches(t.state, target, wild))
      );
    case "postal_code":
      // Postal codes vary (5 vs 9 digit) → a prefix match on the supplied value.
      return allAddr.some(
        (a) => a.postalCode !== null && a.postalCode.trim().toUpperCase().startsWith(target),
      );
    case "enumeration_type":
      return p.enumerationType === value;
    case "taxonomy_description":
      return p.taxonomies.some(
        (t) => t.desc !== null && t.desc.trim().toUpperCase().includes(target),
      );
    default:
      return false;
  }
...
export async function lookupProvider(
  ...
  const text: Partial<Record<(typeof TEXT_FIELDS)[number] | "postal_code", string>> = {};
  for (const f of TEXT_FIELDS) {
    const raw = (args as Record<string, unknown>)[f];
  ...
  if (args.postal_code !== undefined && args.postal_code !== null) {
    ...
    if (pc !== "") {
      if (pc.length > MAX_TEXT_LEN) throw invalidInput("postal_code is too long.");
  ...
  if (args.enumeration_type !== undefined && args.enumeration_type !== null && String(args.enumeration_type).trim() !== "") {
    ...
    if (!ENUM_TYPES.has(enumType)) {
      throw invalidInput(
        `Invalid enumeration_type ${JSON.stringify(enumType)} — expected 'NPI-1' or 'NPI-2'.`,
      );
    }
  }
  ...
  if (args.state !== undefined && args.state !== null && String(args.state).trim() !== "") {
    ...
    if (!NPPES_STATES_SET.has(state)) {
      throw invalidInput(
        `Invalid state ${JSON.stringify(args.state)} — expected a USPS 2-letter state/territory code (a non-state value silently returns 0 on NPPES, indistinguishable from 'no provider').`,
      );
    }
  }
  ...
  if (hasNumber) {
    ...
    if (!NPI_RE.test(npi)) {
      throw invalidInput(
        `Invalid number ${JSON.stringify(npi)} — an NPI is exactly 10 digits (^\\d{10}$).`,
      );
    }
    if (!cmsLuhnValid(npi)) {
      throw invalidInput(
        `NPI ${JSON.stringify(npi)} fails the CMS check-digit (Luhn over 80840+first-9) — it is provably NOT a valid NPI (a typo must never read as 'provider does not exist'). NPPES validates only length, so this pre-check is load-bearing.`,
      );
    }
    ...
    const coFilters: Record<string, string> = {};
    for (const f of CO_FILTER_FIELDS) {
      if (f === "state") {
        if (state !== undefined) coFilters.state = state;
      } else if (f === "enumeration_type") {
        if (enumType !== undefined) coFilters.enumeration_type = enumType;
      } else {
        const tv = (text as Record<string, string | undefined>)[f];
        if (tv !== undefined) coFilters[f] = tv;
      }
    }
    ...
    if (parsed.resultCount === 0) {
      ...
      notes.push(
        `No NPPES record for NPI ${JSON.stringify(npi)} (found:false — the NPI is Luhn-valid but not in the ACTIVE NPPES registry: it may never have been assigned OR was deactivated; NPPES search returns active records). This is an honest not-found, not a fitness determination.`,
      );
      ...
      return withMeta(
        { found: false, provider: null as NppesProvider | null },
        {
          source: SOURCE,
          keylessMode: true,
          returned: 0,
    ...
    let filterMatch: Record<string, boolean> | undefined;
    if (droppedFilters.length > 0) {
      filterMatch = {};
      ...
      if (mismatched.length > 0) {
        notes.push(
          `The NPI is active/enumerated, but it does NOT match your supplied ${mismatched.join(", ")} (checked client-side — see data.filterMatch). A mismatch does NOT mean the NPI is invalid; it means the registry record differs from your expectation for that field.`,
        );
      }
    }
    if (!provider.active) {
      notes.push(
        `provider.active is false (basic.status = ${JSON.stringify(provider.status)}); status is NOT 'A'. Not a fitness determination.`,
      );
    }
    notes.push(NPPES_NOT_DETERMINATION_NOTE, NPPES_REACH_CAP_NOTE);
    ...
    const data: {
      ...
      filterMatch?: Record<string, boolean>;
  ...
  const suppliedRequired = SEARCH_REQUIRED_FIELDS.filter(
    (f) => (text as Record<string, string>)[f] !== undefined,
  );
  ...
  if (!Number.isInteger(limit) || limit < 1 || limit > NPPES_MAX_LIMIT) {
    throw invalidInput(
      `limit ${JSON.stringify(args.limit)} out of range — NPPES caps a page at ${NPPES_MAX_LIMIT} (the API silently clamps >${NPPES_MAX_LIMIT}; this tool rejects it loudly). Use 1..${NPPES_MAX_LIMIT}.`,
    );
  }
  const skip = args.skip ?? 0;
  if (!Number.isInteger(skip) || skip < 0 || skip > NPPES_MAX_SKIP) {
    throw invalidInput(
      `skip ${JSON.stringify(args.skip)} out of range — this vetting tool caps reach at skip ≤ ${NPPES_MAX_SKIP} (a deliberate targeted-lookup POLICY boundary; NPPES itself no longer enforces a skip ceiling). Narrow your filters rather than paging deeper.`,
    );
  }
  ...
  for (const f of SEARCH_REQUIRED_FIELDS) {
    const v = (text as Record<string, string>)[f];
    if (v !== undefined) {
      params.set(f, v);
