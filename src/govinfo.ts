...
export const GOVINFO_PAGE_MARK_RE = new RegExp(
  "^(\\*|[A-Za-z0-9+/=_~.,:%-]{1,4096})$",
);

const GOVINFO_SOURCE = (mode: string) =>
  `${GOVINFO_HOST} via GovInfo API (${mode})`;
...
const OVERLAP_NOTE =
  "GovInfo is the GPO-authoritative BULK-PUBLICATION view (published packages/granules with PDF/XML/MODS downloads + provenance) — complementary to, not a duplicate of, the ecfr_*/fed_register_* tools. For current-text point lookups/search use those; for authoritative published editions, bulk granules, historical point-in-time editions, and non-CFR/FR collections (BILLS, PLAW, CREC, USCODE, BUDGET, GAOREPORTS, …) use GovInfo.";
// m5/m9 — the /collections/{c} date facet filters by lastModified, NOT dateIssued.
const LAST_MODIFIED_NOTE =
  "startDate/endDate filter by lastModified (the record's last-update date), NOT dateIssued (original publication date). A package modified in the window but published earlier IS included; one published in the window but not since modified may be missed by a lastModified query.";
const CURSOR_NOTE =
  "GovInfo uses an opaque cursor: pagination.offset/nextOffset are not meaningful (null). Continue by passing _meta.nextCursor back as the `pageMark` argument; hasMore:false / nextCursor:null means this is the last page.";
 ...
 */
function scrubApiKey(s: string): string {
  if (!/api_key=/i.test(s)) return s;
  try {
    const u = new URL(s);
...
function scrubDeep(v: unknown): unknown {
  if (typeof v === "string") return scrubApiKey(v);
...
// ─── Collection catalog (memoized ~6h; doubles as the validator source) ──
 ...
 */
async function fetchCollections(): Promise<GovinfoCollection[]> {
  const body = await getGovinfo(COLLECTIONS_PATH, "govinfo:/collections", new URLSearchParams());
  const b = (body ?? {}) as { collections?: unknown };
  if (!Array.isArray(b.collections)) {
    throw driftError(
      "govinfo:/collections",
      "govinfo shape drift — /collections response.collections must be an array.",
    );
  }
  return (b.collections as unknown[]).map((c) => {
...
async function collectionsCatalog(): Promise<GovinfoCollection[]> {
 ...
 */
export async function listCollections(_args: Record<string, never> = {}): Promise<MetaBundle> {
...
// ═══════════════════ Tool 2: govinfo_search_packages ══════════════
...
function mapPackage(item: unknown): GovinfoPackage {
 ...
 */
export async function searchPackages(args: GovinfoSearchArgs): Promise<MetaBundle> {
  ...
  const endDate =
    args.endDate !== undefined ? normalizeDate(args.endDate, "endDate", label) : undefined;
  ...
  const known = new Set(
    catalog.map((c) => c.collectionCode).filter((c): c is string => c !== null),
  );
  ...
  const path =
    `${COLLECTIONS_PATH}/${collection}/${startDate}` +
  ...
  const b = (body ?? {}) as { count?: unknown; packages?: unknown; nextPage?: unknown };
  ...
  const packages = (b.packages as unknown[]).map(mapPackage);
  ...
  if (endDate !== undefined) filtersApplied.push("endDate");

  const notes: string[] = [CURSOR_NOTE, LAST_MODIFIED_NOTE];
  if (OVERLAP_COLLECTIONS.has(collection)) notes.push(OVERLAP_NOTE);
  pushKeyNote(notes);
...
function normalizeDate(raw: string, field: string, label: string): string {
 ...
 */
export async function getPackage(args: { packageId: string }): Promise<MetaBundle> {
  ...
  return withMeta(
    { found: true, packageId, package: pkg },
    {
      source: GOVINFO_SOURCE(keyModeLabel()),
      keylessMode: false,
      returned: 1,
