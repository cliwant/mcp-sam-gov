...
function parseScaDocument(doc: string) {
  ...
  const rates: { code: string; title: string; baseRate: number }[] = [];
...
export async function benchmarkLaborRates(args: {
  ...
  for (let page = 2; page <= maxSamplePages; page++) {
    if (rows.length >= totalValue && totalRelation === "eq") break;
    const r = await getJson<CalcResp>(buildUrl(matcher, page), { Accept: "application/json" }, "gsa:calc:page");
    const pageRows = (r.hits?.hits ?? []).map((h) => h._source ?? {});
    if (pageRows.length === 0) break;
    rows.push(...pageRows);
    if (pageRows.length < PAGE_ROWS) break;
  }

  const current = rows.map((r) => r.current_price).filter((n): n is number => typeof n === "number").sort((a, b) => a - b);
  ...
  const notes: string[] = [
    ...
    `Distribution statistics (min/median/max) are computed over the ${rows.length} fetched row(s) (bounded at ${PAGE_ROWS}×${maxSamplePages} pages)${
      rows.length >= matchCount && !saturated
        ? " — the full match set."
